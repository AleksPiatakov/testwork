...
function showPopupResponsev2(res){updateCart();if(typeof checkout!='undefined'){checkout.updateOrderTotals()}}
...
if(FACEBOOK_PIXEL_MODULE_ENABLED){fbq('track','AddToCart',{content_type:'product',content_ids:[data.id],content_name:data.name,content_category:data.category,value:data.price,currency:CURRENCY_CODE})}
...
function updateCart(action){$.post('./ajax_update_cart.php','',function(data){$('#divShoppingCard').replaceWith(data.cart_html);$('.mobile_cart_count').html(data.cart_count);if(action=='add_product')showAlert(RENDER_TEXT_ADDED_TO_CART,$('#divShoppingCard'),'alert-success','alert-dismissible')},'json')}
...
$("#r_spisok img").unveil(500);$('#r_spisok').after(ajax_globals.listing_footer);$('#loadMoreProducts').click(function(){loadMoreProducts()});if(RTPL_PDF_ENABLED)$('#block').prepend(sprintf(RTPL_LISTING_HEADER,ajax_globals.pdf_link))}else{$('#block').html('');$('#attribs').removeClass('pointer_events_none');$('.refresh_icon .fa-spin').fadeOut(20)}},'json')}else if(options="loadMore"){$('#attribs').removeClass('pointer_events_none');$.get(pathSerialize,function(data){var ajax_globals=data.globals;$.each(data,function(k,v){if(v.product_image&&!findWidthToUnveil){findWidthToUnveil=v.product_image.split('&')[1].split('=')[1]}
...
return $(this).on({'mouseenter':handleHover,'mouseleave':handleHover},filter);else return this.on('mouseenter',handleHover).on('mouseleave',handleHover)}})(jQuery);/*! jQuery Form Plugin version: 2.94 (13-DEC-2011) Examples and documentation at: http://malsup.com/jquery/form/ */
...
return;var msg='[jquery.form] '+Array.prototype.join.call(arguments,'');if(window.console&&window.console.log){window.console.log(msg)}else if(window.opera&&window.opera.postError){window.opera.postError(msg)}}})(jQuery);jQuery.autocomplete=function(input,options){var me=this;var currentRequest=null;var $input=$(input).attr("autocomplete","off");if(options.inputClass)$input.addClass(options.inputClass);var results=document.createElement("div");var $results=$(results);$results.hide().addClass(options.resultsClass).css("position","absolute");if(options.width>0)$results.css("width",options.width);$("body").append(results);input.autocompleter=me;var timeout=null;var prev="";var active=-1;var cache={};var keyb=!1;var hasFocus=!1;var lastKeyPressCode=null;function flushCache(){cache={};cache.data={};cache.length=0};flushCache();if(options.data!=null){var sFirstChar="",stMatchSets={},row=[];if(typeof options.url!="string")options.cacheLength=1;for(var i=0;i<options.data.length;i++){row=((typeof options.data[i]=="string")?[options.data[i]]:options.data[i]);if(row[0].length>0){sFirstChar=row[0].substring(0,1).toLowerCase();if(!stMatchSets[sFirstChar])stMatchSets[sFirstChar]=[];stMatchSets[sFirstChar].push(row)}}
...
$('#shoprules').click(function(e){var el='form[name=create_account] input[type=submit]';if(!$(el).hasClass('active_submit'))$(el).addClass('active_submit')});$('.open-menu-xs').click(function(e){$('body').toggleClass('no-scroll-y');$('.header-categories').toggleClass('header-categories-animate');$(this).find('.fader').toggleClass('fader-menu-animate')});function wishlistCompare_xs(e){if($(e).parent().hasClass('wc-box-hovered')){$(e).parent().removeClass('wc-box-hovered')}else{$(e).parent().addClass('wc-box-hovered')}
console.log('opened')}
if($(window).width()<=992){$('#wishlist_box2').find('.wishlist-icon').click(function(e){wishlistCompare_xs(this)});$('#compare_box').find('.compare-icon').click(function(e){wishlistCompare_xs(this)})}
$('[data-toggle="popover"]').on("click",function(e){e.preventDefault()});$(".input_select_all").on("click",function(){$(this).select()});$('.stop_propagation').on('click',function(event){event.stopPropagation()});$('.mainpage-categories-tabs a[data-toggle="tab"]').on('show.bs.tab',function(){$('.mainpage-categories-tabs .tab-content').css('opacity','0.05')
$('.tab-loader').removeClass('hidden');var fName=$(this).data('file');var tabHref=$(this).attr('href');var findWidthToUnveil;var html='';$.get('/ajax_mainpage_tabs.php?fName='+fName,function(data){var ajax_globals=data.globals;$.each(data,function(k,v){if(v.product_image&&!findWidthToUnveil){findWidthToUnveil=v.product_image.split('&')[1].split('=')[1]}
...
if(CATEGORIES_TABS_SLIDER){$("#"+fName).owlCarousel({items:4,responsive:{0:{items:ajax_globals.blocks_num.xs,nav:!0},600:{items:ajax_globals.blocks_num.sm,nav:!0},1000:{items:4,nav:!0,loop:!0}},nav:!0,loop:!0,dots:!1,navText:['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M217.9 256L345 129c9.4-9.4 9.4-24.6 0-33.9-9.4-9.4-24.6-9.3-34 0L167 239c-9.1 9.1-9.3 23.7-.7 33.1L310.9 417c4.7 4.7 10.9 7 17 7s12.3-2.3 17-7c9.4-9.4 9.4-24.6 0-33.9L217.9 256z"></path></svg>','<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M294.1 256L167 129c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.3 34 0L345 239c9.1 9.1 9.3 23.7.7 33.1L201.1 417c-4.7 4.7-10.9 7-17 7s-12.3-2.3-17-7c-9.4-9.4-9.4-24.6 0-33.9l127-127.1z"></path></svg>'],slideSpeed:200,});$("#"+fName).on('changed.owl.carousel',function(e){$("#"+fName+" img").unveil(500)})}
$('#'+fName+" img").unveil(500);$('.mainpage-categories-tabs .tab-content').css('opacity','1')
$('.tab-loader').addClass('hidden')},'json')})
$('.quantity-selector-mask [name="cart_quantity"]').on('change',function(){$('.p_info_item >[name="cart_quantity"]').val($(this).val())})});function openSearchForm(){if($('.main_search_form').hasClass('search-form-open')){closeSearchForm()}else{$('.main_search_form').addClass('search-form-open');$('#searchpr,#searchpr1').addClass('search-form-input-open').select();$('#search-form-button').addClass('search-form-button-open');$('#search-form-button-close').addClass('search-form-button-close-open');$('.search-form-fader').toggleClass('search-form-fader-open');if(!$('#search-form-fader').hasClass('search-form-fader-open')){$('#search-form-fader').addClass('search-form-fader-open')}}}
function closeSearchForm(){$('.main_search_form').removeClass('search-form-open');$('#searchpr,#searchpr1').removeClass('search-form-input-open');$('#search-form-button').removeClass('search-form-button-open');$('#search-form-button-close').removeClass('search-form-button-close-open');if($('#search-form-fader').hasClass('search-form-fader-open')){$('#search-form-fader').removeClass('search-form-fader-open')}}jQuery(document).ready(function(){$(document).on('click','.compare_button',function(event){event.preventDefault();if(typeof $('#compare_counter').html()==='undefined'||$('#compare_counter').html()<4)go_compare($(this).data().id);else showAlert(TEXT_LIMIT_REACHED+'4',$('#divShoppingCard'),'alert-danger','alert-dismissible')})});function go_compare(tovarid){var $input=$('#compare_'+tovarid);var $text=$input.parent().find('label');var $del="";var del=arguments[2]||"";if(del=='')$del=$input.is(':checked')?'delete':'';else $del=del;$.getJSON('./ext/compare/compare.php',{action:'compare',idp:tovarid,ifdel:$del},function(data){$text.html(data.text);$input.is(':checked')?$input.prop('checked',!1):$input.prop('checked',!0);if(window.location.href.toString().split(window.location.host)[1]=='/compare.php')location.reload();$.get('./ext/compare/r_compare_box.php',{method:'ajax',compare_id:tovarid},function(data2){if(data2=='')$('#compare_box').fadeOut(300);else{if(typeof $('#compare_box').html()==='undefined')$('body').prepend('<div id="compare_box"></div>');$('#compare_box').replaceWith(data2);$("#compare_box .lazyload").lazyload()}})})}jQuery(document).ready(function(){$(document).on('click','.wishlisht_button',function(event){event.preventDefault();var $data=$(this).data();var $text=$(this).find('label');var $input=$(this).find('input');$.ajax({url:'./ext/wishlist/ajax_wishlist.php',type:'POST',dataType:'json',data:{request:'wishlist',action:$input.is(':checked')?'delete':'add',id:$data.id},success:function(callback){$text.html(callback.text);$input.is(':checked')?$input.prop('checked',!1):$input.prop('checked',!0);$('[name="wishlist_'+$data.id+'"]').prop('checked',$input.is(':checked'));$.get('./ext/wishlist/r_wishlist_box2.php',{method:'ajax',wishlist_id:$data.id},function(data2){$('#wishlist_box2').replaceWith(data2)})}})})});$(function(){var color_id=$('input[name=color_id]').val();var color_attributes=$('.color_attributes input');color_attributes.on('change',function(event){event.preventDefault();$('#select_id_'+color_id).val($(this).val());$('#select_id_'+color_id).change();$.get('./includes/modules/additional_images2.php',{method:'ajax',pid:jQuery("input[name=products_id]").val(),colid:color_id,col:$(this).val(),color_images:jQuery("#color_images").val()},function(obj){jQuery('.additional_images2').html(obj).promise().done(function(){var sync1=$("#sync1");var isSyncedCarousels=$('#sync1_1').length?!0:!1;$("#sync1 .lazyload, .additional_images2 .lazyload, #sync1_1 .lazyload").lazyload();if(isSyncedCarousels&&typeof syncedCarousel==='function'){carouselClass=new syncedCarousel('#sync1_1','#sync2')}else{$("#sync1").owlCarousel({items:1,loop:!0,dots:!0,slideSpeed:200,nav:!0,navText:['<i class="fa fa-chevron-left"></i>','<i class="fa fa-chevron-right"></i>'],dotsContainer:'#sync2',responsiveRefreshRate:200})}})})})});(function($){var ColorPicker=function(){var
