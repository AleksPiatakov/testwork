...
<head>
    ...
    <link rel="preload" href="/<?php echo DIR_WS_TEMPLATES . TEMPLATE_NAME; ?>/fonts/Roboto-Medium.ttf" as="font"
          type="font/ttf" crossorigin>
    <link rel="preload" href="/<?php echo DIR_WS_TEMPLATES . TEMPLATE_NAME; ?>/fonts/Roboto-Bold.ttf" as="font"
          type="font/ttf" crossorigin>
    <?php
    ...
    <!--[if IE]>
    <link rel="stylesheet" type="text/css" href="<?php echo DIR_WS_TEMPLATES; ?>default/css/ie.css">
    <![endif]-->
...
<div class="page-wrap">
    ...
    <main>
        ...
        if (in_array($content, $no_breadcrumbs_array)) {
            $breadcrumb_content = '<div class="' . $empty_block_class . '"></div>';
        } else {
            $breadcrumb_content = '<div class="title-breadcrumbs">';
            if ($show_breadcrumbs) {
                $breadcrumb_content .= $breadcrumb->trail(' ');
            }
            $breadcrumb_content .= '</div>';
        }
        // BREADCRUMBS ENDS
        ...
        if ($content == 'index_products') {
            ...
            if (!empty($desc_text)) {
                $page_desc = strip_tags(
                    (mb_substr($desc_text, 0, 190, 'UTF-8')) . ((strlen($desc_text) >= 190) ? '...' : '')
                );
            }
            if (!empty($cat_image) and $cat_image != 'categories/') {
                $cat_bg_url = 'style="background-image: url(images/' . $cat_image . ')"';
            }

            ?>
            <div class="category-page-bg_banner" <?= $cat_bg_url ?>>
                <div class="info">
                    <h1 class="category_heading title"><?php echo $current_category['categories_name']; ?></h1>
        ...
        <div class="container">
            <div class="row">
                    ...
                    <div class="col-sm-12">
                        <?php if (is_array($subcat_array)) { ?>
                            <div class="form-group row subcats_imgs">
                                <div class="col-md-12 owl-carousel categories-mainpage<?= $hide_categories_mainpage ? ' hidden' : '' ?>"
                                     id="categories-slider">
                                    <?php // subcategories columns:
                                    foreach ($subcat_array as $cid => $carr) {
                                        if (!empty($carr['img'])) {
                                            // getting img tag to put it into bg-img
                                            $img_tag = $carr['img'];
                                            $img_src = explode('src="', $img_tag);
                                            $img = explode('">', $img_src[1]);
                                            $img = str_replace('images/', 'getimage/512x512/', $img[0]);
                                            $img_bg = 'style="background-image: url(' . $img . ');"';
                                        } else {
                                            $color_array = array(
                                            ...
                                            $rand_colors = $color_array[rand(0, count($color_array) - 2)];
                                            $img_bg = 'style="' . $rand_colors . '"';
                                        }
                                        ...
                                        echo '
                                    <div class="item">
                                        <a href="' . $carr['url'] . '" title="' . $carr['name'] . '">
                                            <div class="thumb" ' . $img_bg . '></div>
                                            <span class="title">' . $carr['name'] . '</span>
                                        </a>
                                    </div>';
                                    }
                                    ?>
                ...
                <?php if ($sidebar_left) : ?>
                    ...
                    <?php if (isMobile()) {
                        echo '<button type="button" class="sidebar-toggle-back hidden-xs hidden-sm"></button>';
                    } ?>
                <?php endif ?>
                <!-- END COLUMN LEFT -->
