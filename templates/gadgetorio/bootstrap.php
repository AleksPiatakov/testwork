...
foreach ($no_breadcrumbs_array as $key => $val) {
    if ($content == $val) {
        $empty_block_class = 'emprty_title';
...
if ($content == 'create_account' || $content == 'create_account_success' || $content == 'login') {
    $show_breadcrumbs = false;
}
...
$assets->css[] = DIR_WS_TEMPLATES . TEMPLATE_NAME . '/css/simplebar.css';
$assets->css[] = DIR_WS_TEMPLATES . TEMPLATE_NAME . '/css/fonts.css';
$assets->css[] = DIR_WS_TEMPLATES . TEMPLATE_NAME . '/css/popup_cart.css';

list($r, $g, $b) = sscanf($template->getMainconf('MC_COLOR_2'), "#%02x%02x%02x");
$second_color_rgb = $r . ',' . $g . ',' . $b;
$assets->cssInline[] = ':root {
            --sm-image-height: ' . SMALL_IMAGE_HEIGHT . 'px;
            --sm-second-color-rgb: ' . $second_color_rgb . ';
      ...
      .p_img_href {height: ' . (SMALL_IMAGE_HEIGHT - 0) . 'px;line-height: ' . (SMALL_IMAGE_HEIGHT - 0) . 'px;}
      .p_img_href_list{max-width: ' . (SMALL_IMAGE_WIDTH - 60) . 'px;}
      .product {height:' . (SMALL_IMAGE_HEIGHT + 160) . 'px;}
      .product_slider {height:' . (SMALL_IMAGE_HEIGHT + 170) . 'px;}';
