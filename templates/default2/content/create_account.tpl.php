<div class="row">
    <div class="col-md-6">
        ...
        <?php if (ACCOUNT_DOB === 'true') : ?>
            <div class="form-group">
                <input type="text" name="dob" class="form-control reg_input" placeholder="<?= ENTRY_DATE_OF_BIRTH; ?>"
                       value="<?= $_POST['dob']; ?>">
            </div>
        <?php endif ?>
        ...
        <div class="form-group">
            <input type="text" name="email_address" class="form-control reg_input"
                   placeholder="<?= ENTRY_EMAIL_ADDRESS; ?>" value="<?= $_POST['email_address']; ?>">
        </div>
        <?php if (ACCOUNT_STREET_ADDRESS === 'true') : ?>
            <div class="form-group">
                <input type="text" name="street_address" class="form-control reg_input"
                       placeholder="<?= ENTRY_STREET_ADDRESS; ?>" value="<?= $_POST['street_address']; ?>">
            </div>
        <?php endif ?>
        ...
        if (ACCOUNT_COUNTRY === 'true' or ACCOUNT_STATE === 'true') {
            ...
            echo '<div class="form-group" ' . $non_show . '>' . tep_get_country_list(
                'selectCountry',
                $_POST['selectCountry'] ?: STORE_COUNTRY,
                'data-zone="' . ($_POST['selectRegion'] ?: STORE_ZONE) . '" class="checkout_inputs required form-control"'
            ) . '</div>';
        }
        ...
        if (ACCOUNT_TELE === 'true') : ?>
            <div class="form-group">
                <input type="text" name="telephone" class="form-control reg_input"
                       placeholder="<?= ENTRY_TELEPHONE_NUMBER; ?>" value="<?= $_POST['telephone']; ?>">
            </div>
        <?php endif ?>
        ...
        <?php endif ?>
        <div class="form-group">
            <?= tep_draw_checkbox_field('shoprules', '1', false, 'id="shoprules"') ?>
        ...
        <div class="form-group">
            <input type="submit" class="btn btn-default" value="<?= SEND_MESSAGE; ?>">
        </div>
        </form>
    </div>
</div>
