<table border="0" width="100%" cellspacing="0" cellpadding="<?php echo getConstantValue('CELLPADDING_SUB', '0'); ?>">
    ...
    if ($_GET['action'] == 'archive') {
        $delete_query = tep_db_query(
            "select orders_products_download_id, orders_id from orders_products_download where orders_products_download_id = '" . $_GET['aID'] . "'"
        );
        $delete = tep_db_fetch_array($delete_query);
        tep_db_query(
            "update orders_products_download set archived = 'yes' where orders_id = '" . $delete['orders_id'] . "' AND orders_products_download_id = '" . $_GET['aID'] . "' LIMIT 1"
        );
        ?>
                            ...
                            <tr>
                                <td>
                                    <table border="0" width="100%" cellspacing="1" cellpadding="2">
                                        <tr>
                                            <td align="center"><?php echo TEXT_DOWNLOAD_NUMBER ?></td>
                                            <td align="center"><?php echo HEADING_DOWNLOAD_HERE ?></td>
                                            <td align="center"><?php echo TABLE_HEADING_DOWNLOAD_DATE ?></td>
                                            <td align="center"><?php echo TABLE_HEADING_DOWNLOAD_COUNT ?></td>
                                        </tr>
                                        <!-- list of products -->
                                        <?php
                                        while ($downloads_values = tep_db_fetch_array($downloads_query)) {
                                            list($dt_year, $dt_month, $dt_day) = explode(
                                                '-',
                                                $downloads_values['date_purchased_day']
                                            );
                                            $download_timestamp = mktime(
                                                23,
                                                59,
                                                59,
                                                $dt_month,
                                                $dt_day + $downloads_values['download_maxdays'],
                                                $dt_year
                                            );
                                            $download_expiry = date('Y-m-d H:i:s', $download_timestamp);

                                            if (
                                                ($downloads_values['download_count'] > 0) && (file_exists(
                                                ...
                                                echo '            <td align="center"><a href="' . tep_href_link(
                                                    FILENAME_DOWNLOAD,
                                                    'order=' . $downloads_values['orders_id'] . '&id=' . $downloads_values['orders_products_download_id']
                                                ) . '"><b><font color=red>' . TEXT_DOWNLOAD_HERE . '</font></b></a></td>' . "\n";
                                                echo '            <td align="center">' . tep_date_long(
                                                    $download_expiry
                                                ) . '</td>' . "\n";
