...
<div class="small_slider" id="manufacturers">
    <div>
        ...
        $manufacturers_query = tep_db_query(
            "SELECT DISTINCT m.`manufacturers_image`, m.`manufacturers_id`, mi.`manufacturers_name` FROM " . TABLE_MANUFACTURERS . " `m` JOIN " . TABLE_MANUFACTURERS_INFO . " `mi` on m.manufacturers_id = mi.manufacturers_id  WHERE `status` = '1' and mi.languages_id = $languages_id ORDER BY `manufacturers_name` LIMIT {$limit}"
        );
        $output_man = '';
