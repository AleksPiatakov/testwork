...
if (isset($_SESSION['compares']) && (count($_SESSION['compares']) > 0)) {
    ...
    <div id="compare_box">
        ...
        <div class="clearfix">
            <a class="gocomparelink" href="<?php echo tep_href_link('compare.php'); ?>"><?php echo COMPARE; ?></a>
        </div>
        <?php
        ...
        foreach ($_SESSION['compares'] as $key => $val) {
            ...
            // Delete btn
            $compare_html .= '<a class="compare_block__delete" onclick="go_compare(' . $comp['products_id'] . ',' . $current_category_id . ',\'delete\')" href="javascript:;" ><i class="icon-remove-compare"><i class="fa fa-trash"></i></i></a>';
            $compare_html .= '</div>';
            $compare_html .= '</div>';
