...
if (isset($_SESSION['compares']) && (count($_SESSION['compares']) > 0)) {
    ...
    <table border="0" cellspacing="0" cellpadding="0" class="compare_table">
        ...
        foreach ($cps as $key => $val) {
            ...
            if ($new_price = tep_get_products_special_price($val['products_id'])) {
                ...
                if ($query_special_prices_hide_result == 'true') {
                    $products_price = $currencies->display_price($new_price, tep_get_tax_rate($val['products_tax_class_id']));
                    echo '<td><div class="prod_price">' .
                        '<span class="old_price_compare_product">' . ((!empty($new_price)) ? $products_price . ' ' : '') . '</span>' .
                        ((!empty($new_price)) ? $new_price : $products_price)
                        . tep_draw_hidden_field('products_id', $comp['products_id']) . '</div></td>';
                } else {
                    $spec_price = $new_price;
                    $products_price = $currencies->display_price($new_price, tep_get_tax_rate($val['products_tax_class_id']));
                    if ($new_price != $val['products_price']) {
                        $products_price =$currencies->display_price($val['products_price'], tep_get_tax_rate($val['products_tax_class_id']));
                    }
                    echo '<td><div class="prod_price">' . ((!empty($new_price)) ? $products_price . ' ' : '') . '</span>'
                        . tep_draw_hidden_field('products_id', $comp['products_id']) . '</div></td>';
                }
            } else {
                $products_price = $currencies->display_price($val['products_price'], tep_get_tax_rate($val['products_tax_class_id']));
