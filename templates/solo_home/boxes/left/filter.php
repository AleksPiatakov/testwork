...
if ((!empty($_GET['cPath']) or isset($_GET['manufacturers_id']) or isset($_GET['keywords'])) and ATTRIBUTES_PRODUCTS_MODULE_ENABLED == 'true') {
    if (defined('SEO_FILTER') && constant('SEO_FILTER') == 'true') {
        require(DIR_WS_TEMPLATES . 'solo_home/boxes/left/filter_seo.php');
    } else {
        global $currency;
        if (!isset($config)) {
                ...
                if ($config['price_range']['val'] == 1) {
                    ...
                    $priceRangeFilterHtml .= '<div class="dipcen">
                         ...
                         <span class="left slider-from">
                           <input type="number" min="0" class="input-type-number-custom" name="rmin" id="range1" data-val="' . (!empty($_GET['rmin']) ? $_GET['rmin'] : $listing_sql_max['min_price'] + 0) . '" value="' . trim(
                            $currencies->currencies[$currency]['symbol_left']
                        ) . (!empty($_GET['rmin']) ? $_GET['rmin'] : $listing_sql_max['min_price'] + 0) . '&nbsp;' . trim(
                            $currencies->currencies[$currency]['symbol_right']
                        ) . '"  />
                         </span>
                         <span class="left slider-to">                                                                                                             
                           <input type="number" min="0" class="input-type-number-custom" name="rmax" id="range2" data-val="' . (!empty($_GET['rmax']) ? $_GET['rmax'] : $listing_sql_max['max_price'] + 0) . '" value="' . trim(
                            $currencies->currencies[$currency]['symbol_left']
                        ) . (!empty($_GET['rmax']) ? $_GET['rmax'] : $listing_sql_max['max_price'] + 0) . '&nbsp;' . trim(
                            $currencies->currencies[$currency]['symbol_right']
                        ) . '" />
                         </span>
                         <br><br>
