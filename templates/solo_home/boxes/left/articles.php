...
if (is_array($art_array)) {
    foreach ($art_array as $articles) {
        if (!empty($articles['image'])) {
            ...
            $all_articles_string .= '<li><img class="lazyload" src="' . DIR_WS_IMAGES_CDN . 'pixel_trans.png" data-src="' . $image . '" alt="' . $articles["name"] . '"><a href="' . $articles['link'] . '"><span>' . $articles['name'] . '</span></a></li>';
        } else {
    ...
    <!-- ARTICLES -->
    <div class="articles">
        <div class="like_h2"><?php echo $topicName;?></div>
        <nav>
            <ul>
