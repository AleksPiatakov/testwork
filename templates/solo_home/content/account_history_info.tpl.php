<?php ob_start();  //Buffer ?>
    ...
    <div id="account_order_info">
        <div class="row order_header">
            <div class="col-sm-5"><strong><?php echo HEADING_ORDER_DATE . tep_date_long(
                    ...
                    <?php if (tep_not_null($order->info['shipping_method'])) : ?>
                        ...
                        for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
                            echo '<div class="row account_delivery_method">' . "\n" .
                                '<div class="col-sm-8 col-xs-6">' . $order->totals[$i]['title'] . '</div>' . "\n" .
                                '<div class="col-sm-4 col-xs-6">' . $order->totals[$i]['text'] . '</div>' . "\n" .
                                '</div>' . "\n";
                        }
                        ?>
                    <?php endif; ?>
                    ...
                    <?php foreach ($order->products as $product) : ?>
                        ...
                        <div class="col-sm-3 <?php echo (sizeof(
                            ...
                            <strong><?php echo $currencies->format(
                                tep_add_tax($product['final_price'], $product['tax']) * $product['qty'],
                                true,
                                $order->info['currency'],
