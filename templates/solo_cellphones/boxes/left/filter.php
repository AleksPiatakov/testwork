...
if ((!empty($_GET['cPath']) or isset($_GET['manufacturers_id']) or isset($_GET['keywords'])) and ATTRIBUTES_PRODUCTS_MODULE_ENABLED == 'true') {
    if (defined('SEO_FILTER') && constant('SEO_FILTER') == 'true') {
        require(DIR_WS_TEMPLATES . 'solo_cellphones/boxes/left/filter_seo.php'); // при необходимости создать отдельный файл для шаблона
    } else {
        global $currency;
        if (!isset($config)) {
        ...
        <div id="filters_box" class="box filter_box">
            <span class="sidebar-box-title">Filters</span>
            <div class="filter_box_in">
                ...
                if ($config['price_range']['val'] == 1) {
                    ...
                    // Price Range Filter:
                    $filter_slider_currency = '<span class="price_fltr">' . ($currencies->currencies[$currency]['symbol_left'] ? $currencies->currencies[$currency]['symbol_left'] : $currencies->currencies[$currency]['symbol_right']) . '</span>';
                    $priceRangeFilterHtml .= '<div class="dipcen">
                         <div class="filter-header">
                            <span class="filter_heading">' . COMP_PROD_PRICE . ':' . $filter_slider_currency . '</span>
                             <div class="slider-range-form">
...
//                        $class_collapsed = 'in';
//                    }
                            $class_collapsed = isset($_GET[$at_id]) ? 'in' : '';
                            if (is_array($show_in_filter) and in_array($at_id, $show_in_filter)) {
                                $attributesFilterHtml .= '<div class="filter_accordion">';
                                $attributesFilterHtml .= '<span class="filter_heading" data-toggle="collapse" data-target="#filter_' . $filter_counter . '">' . $attr_names_array[$at_id] . '</span>';
                                $attributesFilterHtml .= '<div class="filter_content collapse ' . $class_collapsed . '" id="filter_' . $filter_counter . '">';
                                ...
                                if (is_array($attr_vals_array[$at_id])) {
                                    foreach ($attr_vals_array[$at_id] as $at_val_id => $at_val_name) {
                                        ...
                                        $attributesFilterHtml .= '<div class="item">
                                                         ...
                                                         <label for="' . $at_val_id . '2"><span class="attr_name">' . $at_val_name . $counts_number . '</span><span class="counter_attr_products">123</span></label>
                                                       </div>';
                                        //}
                                    }
                                }
