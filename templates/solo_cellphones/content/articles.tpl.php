...
// check to see if there are deeper topics within the current topic
        ...
        foreach ($topic_links as $topic_link_id) {
            $topics_query = tep_db_query(
                "select count(*) as total from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.parent_id = '" . (int)$topic_link_id . "' and t.topics_id = td.topics_id and td.language_id = '" . (int)$languages_id . "'");
            $topics = tep_db_fetch_array($topics_query);
            if ($topics['total'] < 1) {
                // do nothing, go through the loop
            } else {
                $topics_query = tep_db_query(
                    "select t.topics_id, td.topics_name, t.parent_id from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.parent_id = '" . (int)$topic_link_id . "' and t.topics_id = td.topics_id and td.language_id = '" . (int)$languages_id . "' order by sort_order, td.topics_name");
...
// We show them all
    ...
    <td width="100%" valign="top">
        <table border="0" width="100%" cellspacing="0" cellpadding="0">
            ...
            if (($articles_all_split->number_of_rows > 0) && ((defined(
                        'ARTICLE_PREV_NEXT_BAR_LOCATION') && (ARTICLE_PREV_NEXT_BAR_LOCATION == 'top' || ARTICLE_PREV_NEXT_BAR_LOCATION == 'both')))) {
                ?>
                <tr>
                    <td>
...
<div>
    ...
    <div
            class="right"> <?php echo TEXT_RESULT_PAGE . ' ' . $articles_all_split->display_links(
                MAX_DISPLAY_PAGE_LINKS,
