...
if ($cart->count_contents() > 0) { ?>
    ...
    for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
        ...
        if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
            ...
            $products[$i]['attributes'] = array_filter($products[$i]['attributes'], function ($k) { return is_numeric($k); });
            if (!empty($products[$i]['attributes'])) {
                $optionTypesQuery = tep_db_query(
                    "select distinct pa.options_id, pa.options_values_id, pa.pa_imgs
                ...
                where pa.products_id = " . (int)$products[$i]['id'] . " and pa.options_values_id in (" . implode(
                        ',',
                        $products[$i]['attributes']
                    ) . ")"
                );
                $attributeImages = $products_attribute_image = '';
                if (tep_db_num_rows($optionTypesQuery) > 0) {
                ...
                foreach ($products[$i]['attributes'] as $option => $value) {
                    $products_name .= '<li><span>' . $products[$i][$option]['products_options_name'] . '</span> : ' . $products[$i][$option]['products_options_values_name'] . '</li>' .
                        tep_draw_hidden_field(
                            'id[' . $products[$i]['id'] . '][' . $option . ']',
                            $value,
                            'class="attr-hidden-value"'
                        );
                }
                $products_name .= '</ul>';
            }
            ...
            <div class="checkout_cart_item" data-id="<?= $value["id"] ?>">
                ...
                <div class="cart_item_info">
                    ...
                    <div class="cart_calc_cost">
                        <input type="hidden" name="products_id[]" value="<?= $value['id'] ?>"> <input type="number"
                                                                                                      value="<?= $value['qty'] ?>"
...
<div class="coupon_newsletter_block">
    ...
    <?php if (CUPONES_MODULE_ENABLED == 'true') {
        ...
        <div class="coupon_block">
            <input type="checkbox" id="show_coupon" <?= $checkboxState; ?>><label
                    for="show_coupon"><?= NEW_CHECKOUT_COUPON_TEXT; ?></label>
