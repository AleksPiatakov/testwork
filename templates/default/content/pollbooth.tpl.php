...
$location = ' : <a href="' . tep_href_link(
        ...
        'op=results',
        'NONSSL'
    ) . '" class="headerNavigation"> ' . getConstantValue('NAVBAR_TITLE_1', 'NAVBAR_TITLE_1') . '</a>';
                        ...
                        switch ($_GET['op']) {
                            case "results":
                                ...
                                $poll_query = tep_db_query(
                                    "SELECT pollid, timeStamp FROM " . TABLE_PHESIS_POLL_DESC . " WHERE pollid=" . (int)$pollid);
                                $polls = tep_db_fetch_array($poll_query);
                                $title_query = tep_db_query(
                                    "SELECT optionText from " . TABLE_PHESIS_POLL_DATA . " where pollid=" . (int)$pollid . " and voteid=0 and language_id = " . (int)$languages_id);
                                ...
                                <?php
                                $query = "SELECT SUM(optionCount) AS sum FROM " . TABLE_PHESIS_POLL_DATA . " WHERE pollid=" . (int)$pollid . " and language_id = " . (int)$languages_id;

                                $result = tep_db_query($query);
                                $polls = tep_db_fetch_array($result);
                                $sum = $polls['sum'];
                                for ($i = 1; $i <= 15; $i++) {
                                    $query = "SELECT pollid, optiontext, optioncount, voteid FROM " . TABLE_PHESIS_POLL_DATA . " WHERE (language_id = " . (int)$languages_id . ") and (pollid=" . (int)$pollid . ") AND (voteid=" . (int)$i . ")";
                                    $result = tep_db_query($query);
                                    $polls = tep_db_fetch_array($result);
                                            ...
                                            <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                                <tr>
                                                    <td class="smallText"><?php echo $comments_split->display_count(
                                                            ...
                                                            $_GET['page'],
                                                            TEXT_DISPLAY_NUMBER_OF_COMMENTS
                                                        ); ?></td>
                                                    <td align="right"
                                ...
                                <tr>
                                    <td colspan="2" align="center" class="main">
                                        <div class="form-group">
                                            <?php echo getConstantValue("_TOTALVOTES") ?> <span
                                            ...
                                            <?php if (defined('SHOW_POLL_COMMENTS') && SHOW_POLL_COMMENTS == '1') { ?>
                                                <a class="btn btn-xs btn-default"
                                                   href="<?php echo tep_href_link(
                                                       ...
                                                       'NONSSL'
                                                   ) ?>"><?php echo getConstantValue("_ADD_COMMENTS") ?>
                                                </a>
                                           ...
                                           href="<?php /*echo tep_href_link('pollbooth.php', 'pollid=' . $pollid . '&op=vote', 'NONSSL') */
                                            ...
                                            <a class="btn btn-xs btn-default"
                                               href="<?php echo tep_href_link(
                                                   ...
                                                   'op=list',
                                                   'NONSSL'
                                               ) ?>"><?php echo getConstantValue("_OTHERPOLLS") ?></a>
                                        </div>
                                    </td>
                                </tr>
                                <?php
                            ...
                            case 'list':
                                ...
                                while ($polls = tep_db_fetch_array($result)) {
                                    ...
                                    $title_query = tep_db_query(
                                        "select optionText from " . TABLE_PHESIS_POLL_DATA . " where pollid=" . (int)$id_poll . " and voteid=0 and language_id = " . (int)$languages_id);
                                    $title = tep_db_fetch_array($title_query);
                                    $fullresults = "<a href=\"" . tep_href_link(
                                            'pollbooth.php',
                                            'op=results&pollid=' . $id_poll,
                                            'NONSSL'
                                        ) . "\">" . getConstantValue("_POLLRESULTS") . "</a>";
                                    $result1 = tep_db_query(
                                        "SELECT SUM(optioncount) AS sum FROM " . TABLE_PHESIS_POLL_DATA . " WHERE pollid=" . (int)$id_poll . " and language_id = " . (int)$languages_id);
                                    ...
                                    if ($polls['poll_type'] == '0') {
                                        echo("<td class=\"main\">" . getConstantValue("_PUBLIC") . "</td>");
                                    } else {
                                        echo("<td class=\"main\">" . getConstantValue("_PRIVATE") . "</td>");
                                    }
                                    if ($polls['poll_open'] == '0') {
                                        echo("<td class=\"main\">" . getConstantValue("_POLLOPEN") . "</td>");
                                    } else {
                                        echo("<td class=\"main\">" . getConstantValue("_POLLCLOSED") . "</td>");
                                    }
                            ...
                            case "vote":
                                ...
                                $poll_query = tep_db_query(
                                    "select voters from " . TABLE_PHESIS_POLL_DESC . " where pollid=" . (int)$pollid);
                                ...
                                for ($i = 1; $i <= 12; $i++) {
                                    $query = tep_db_query(
                                        "select pollid, optiontext, optioncount, voteid from " . TABLE_PHESIS_POLL_DATA . " where (pollid=" . (int)$pollid . ") and (voteid=" . (int)$i . ") and (language_id=" . (int)$languages_id . ")");
                                ...
                                $query = tep_db_query(
                                    "select sum(optioncount) as sum from " . TABLE_PHESIS_POLL_DATA . " where pollid=" . (int)$pollid . " and language_id = " . (int)$languages_id);
                                if ($result = tep_db_fetch_array($query)) {
                                    $sum = $result['sum'];
                                }
                                $content .= "[ <a href=\"" . tep_href_link(
                                        ...
                                        'NONSSL'
                                    ) . "\">" . _RESULTS . "</a> | <a href=\"" . tep_href_link(
                                        'pollbooth.php',
                                        'op=list',
                                        'NONSSL'
                                    ) . "\">" . _OTHERPOLLS . "</a> ]";
