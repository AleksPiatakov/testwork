...
if (isset($_POST['render']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest' || !$module_is_ajax) {
    ...
    $cols = !isMobile() ? $config['cols']['val'] : 2;
    $limit = isMobile() ? $item_limit_mobile : $item_limit;
    $manufacturers_query = tep_db_query("SELECT DISTINCT m.`manufacturers_image`, m.`manufacturers_id`, mi.`manufacturers_name` FROM " . TABLE_MANUFACTURERS . " `m` LEFT JOIN " . TABLE_MANUFACTURERS_INFO . " `mi` on m.manufacturers_id = mi.manufacturers_id  WHERE `m`.`status` = '1' and `m`.`manufacturers_featured` = 1 and mi.languages_id = $languages_id ORDER BY `m`.`sort`, `mi`.`manufacturers_name` LIMIT {$limit}");
    $output_man = ''; ?>
    <div class="small_slider" id="manufacturers" data-carousel-cols="<?= $cols ?>">
        <div>
            <?php
...
$assets->jsVariables[] = 'window.manufacturersCarouselCols = document.getElementById("manufacturers") != null ? parseInt(document.getElementById("manufacturers").dataset.carouselCols) : 7;';
?>
