...
function productInCart(cartKey){$('#r_buy_intovar button[type="submit"]').attr('disabled');$.post('./ajax_product_in_cart.php','cartKey='+cartKey,function(data){$('#r_buy_intovar').html(data.button_html)},'json')}
...
function calculate_sum(selected_op){var summ,old_summ,sel_ops;setTimeout(function(){if($('input[name=prod_price]').val()!='-'){if(selected_op.attr('data-prefix')=='='&&$('#id_option_other'+selected_op.val()).val()!='=0'){summ=($('#id_option_other'+selected_op.val()).val()).substring(1)}else{$('.select_id_option').each(function(){sel_ops=$(this).parent().find($("select option:selected"));if(sel_ops.attr('data-prefix')=='='&&$('#id_option_other'+sel_ops.val()).val()!='=0'){summ=($('#id_option_other'+sel_ops.val()).val()).substring(1);return!1}});if(!summ)summ=parseFloat($('input[name=prod_price]').val())}
...
modal({id:'ContactUs',body:response.msg,render:!0,after:function(modal){}})},'json')});jQuery('body').on('click touchstart','.popup_cart',function(event){event.preventDefault();if($('[data-dismiss="modal"]')){$('[data-dismiss="modal"]').click();setTimeout(function(){showCartpopup()},300)}else{showCartpopup()}});jQuery('.call_us_btn').click(function(e){e.preventDefault();pop_contact_us()});$('#user-login-dropdown .dropdown-menu').on('click',function(event){event.stopPropagation()});$(document).ready(function(){if($(window).width()>768){$('[data-toggle="tooltip"]').tooltip({container:'body'})}
...
$(document).on('keyup','input[name=phone]',function(e){this.value=this.value.replace(/[^0-9\-\+()]/g,'')});﻿;(function($){$.fn.superfish=function(op){var sf=$.fn.superfish,c=sf.c,$arrow=$('<span class="'+c.arrowClass+'"> &#187;</span>'),over=function(e){var $$=$(this),menu=getMenu($$);if(e.type==='mouseenter'||e.type==='focusin'){$$.children('a').data('follow',!0)}
...
function startGoogleOAuth(){var script='&lt;script src="https://apis.google.com/js/client:platform.js?onload=googleStartAuth" async defer>&lt;\/script>';$('body').append(script.replace(/&lt;/g,'<'))};$(document).on('click','.googleSigninButton',startGoogleOAuth);if(typeof syncedCarousel=="undefined"){var carouselClass;function syncedCarousel(car1,car2){this.sync3=$(car1);this.sync4=$(car2);this.slidesPerPage=1;this.syncedSecondary=!0;this.state=!1;this.sync4OwlInstance;this.initCarousels()}
syncedCarousel.prototype.syncPosition=function(el){var count=el.item.count-1;var current=el.item.index-1;if(current<0){current=count}
if(current>count){current=0}
carouselClass.sync4.find(".owl-item").removeClass("current").eq(current).addClass("current");var onscreen=carouselClass.sync4.find('.owl-item.active').length-1;var start=carouselClass.sync4.find('.owl-item.active').first().index();var end=carouselClass.sync4.find('.owl-item.active').last().index();if(current>end){carouselClass.sync4.trigger('to.owl.carousel',[current,100,!0])}
if(current<start){carouselClass.sync4.trigger('to.owl.carousel',[current-onscreen,100,!0])}};syncedCarousel.prototype.syncPosition2=function(el){if(this.syncedSecondary){var number=el.item.index;carouselClass.sync3.trigger('to.owl.carousel',[number,100,!0])}};syncedCarousel.prototype.initCarousels=function(){var syncclass=this;this.sync3.owlCarousel({items:1,slideSpeed:2000,dots:!0,loop:!0,nav:!0,navText:['<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M34.52 239.03L228.87 44.69c9.37-9.37 24.57-9.37 33.94 0l22.67 22.67c9.36 9.36 9.37 24.52.04 33.9L131.49 256l154.02 154.75c9.34 9.38 9.32 24.54-.04 33.9l-22.67 22.67c-9.37 9.37-24.57 9.37-33.94 0L34.52 272.97c-9.37-9.37-9.37-24.57 0-33.94z"></path></svg>','<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"></path></svg>'],responsiveRefreshRate:200,}).on('changed.owl.carousel',this.syncPosition);this.sync4.on('initialized.owl.carousel',function(el){syncclass.sync4.find(".owl-item").eq(0).addClass("current");var index_pos=el.page.size;var length=syncclass.sync4.find('.owl-item').length;var first_el=syncclass.sync4.find('.owl-item').first().index();var last_el=syncclass.sync4.find('.owl-item').last().index();$("#sync4 .owl-next").on("click",function(event){++index_pos;if(index_pos>length){syncclass.sync4.trigger('to.owl.carousel',[first_el,100,!0]);index_pos=el.page.size}});$("#sync4 .owl-prev").on("click",function(){--index_pos;if(index_pos<el.page.size){syncclass.sync4.trigger('to.owl.carousel',[last_el,100,!0]);index_pos=length}})});let hideNavIf3items=!0;let prodPhotoCount=syncclass.sync3.find('.owl-item').length;if(prodPhotoCount<=5){hideNavIf3items=!1}
this.sync4OwlInstance=this.sync4.owlCarousel({items:this.slidesPerPage,dots:!1,nav:hideNavIf3items,smartSpeed:200,slideSpeed:500,navText:['<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M34.52 239.03L228.87 44.69c9.37-9.37 24.57-9.37 33.94 0l22.67 22.67c9.36 9.36 9.37 24.52.04 33.9L131.49 256l154.02 154.75c9.34 9.38 9.32 24.54-.04 33.9l-22.67 22.67c-9.37 9.37-24.57 9.37-33.94 0L34.52 272.97c-9.37-9.37-9.37-24.57 0-33.94z"></path></svg>','<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"></path></svg>'],responsive:{0:{items:3,margin:5},480:{margin:10},991:{items:3},},slideBy:1,});this.sync4.on('changed.owl.carousel',this.syncPosition2);this.sync4.on("click",".owl-item",function(e){e.preventDefault();var number=$(this).index();syncclass.sync3.trigger('to.owl.carousel',[number,100,!0])});this.sync4OwlInstance.on("changed.owl.carousel",function(e){this.state=!0})}}
$(document).on('click',".product-modal-button",function(e){e.preventDefault();$.ajax({url:"ext/product_modal/product_modal.php",method:"get",data:{'product_href':$(this).parent().attr('href')},success:function(data){let json=JSON.parse(data);modal({body:json.product.markup,title:json.product.products_name,width:1140,freeShipping:json.product.products_free_ship,unique_styles:['product_modal'],});$('.lazyload').addClass('anim');if(!$("body").find("#review_scripts_loaded").val()){$("body").append("<input type='hidden' id = 'review_scripts_loaded' value = '1'>");$("body").append(json.product.comments.script)}
$('select.select_attr_select').selectize({maxItems:1});$('.productModal-tabItem:first-child').click();carouselClass=new syncedCarousel('#sync3','#sync4');$('.actualPrice').addClass('new_price_card_product');$('.owl-carousel img').lazyload();calculate_sum($('.select_id_option:first'));if(typeof includeRecaptchaFile=='function'){includeRecaptchaFile()}}})});$(".pagination pagination-sm").find("a").click(function(e){e.preventDefault()})
$('body').on('click','.productModal-tabItem',function(e){e.preventDefault();$('.productModal-tabItem').removeClass('productModal-tabItem_active');$('.productModal-tabContent-Item').removeClass('productModal-tabContent-Item_active');$(this).addClass('productModal-tabItem_active');$($(this).attr('href')).addClass('productModal-tabContent-Item_active')})
$("#modal_qty_input").on("input",function(){$(this).parent().find(".add2cart").attr("data-qty",$(this).val())});'use strict';function addHookie($key,$value,$sortOrder=10){if(typeof hookie!='undefined'){if(typeof hookie[$key]!='object'){hookie[$key]={}}
...
for(var tagIndex=0;tagIndex<formElements.length;tagIndex++){for(var elementIndex=0;elementIndex<formElements[tagIndex].length;elementIndex++){formElements[tagIndex][elementIndex].setAttribute('disabled',!0)}};makeJSConstantsFromJson();function gtag(){return!1};var productsPricesForAnalytics=document.querySelector(".productsPricesForAnalytics")?document.querySelector(".productsPricesForAnalytics").value:"";var productsIdsForAnalytics=document.querySelector(".productsIdsForAnalytics")?document.querySelector(".productsIdsForAnalytics").value:"";if(page_name=="index_products"&&searchKeywords!=""&&productsIdsForAnalytics!=""&&productsPricesForAnalytics!=""){gtag('event','page_view',{'send_to':'',"ecomm_prodid":productsIdsForAnalytics,"ecomm_pagetype":"searchresults","ecomm_totalvalue":productsPricesForAnalytics})};if(page_name=="checkout_success"){var successPageProductsIds=document.getElementById("successPageProductsIds")?document.getElementById("successPageProductsIds").value:"";var successPageProductsPrices=document.getElementById("successPageProductsPrices")?document.getElementById("successPageProductsPrices").value:"";gtag('event','page_view',{'send_to':'',"ecomm_prodid":successPageProductsIds,"ecomm_pagetype":"purchase","ecomm_totalvalue":successPageProductsPrices});drawEcommerce()};function fbq(){return!1};;;$('.ajax-module-box').each(function(){mainPageModules.push($(this).attr('data-module-id'))});makeMainSlider();if($(window).width()>'991'){setTimeout(function(){$('.custom_panel_block').addClass('visible');$('.open_custom_panel_btn').removeClass('anim')},500)}else{$('.open_custom_panel_btn').addClass('anim')}
...
if(page_name=='create_account_success'){doHookie('complete_registration')};function make_featured_slider(){var sliderObject=document.getElementById("featured");var itemsCount=$(">div",sliderObject).length;var it0=2;var it600=2;var it992=6;var it1200=4;var it1600=6;var navValue0=it0<itemsCount;var navValue600=it600<itemsCount;var navValue992=it992<itemsCount;var navValue1200=it1200<itemsCount;var navValue1600=it1600<itemsCount;$("#featured").owlCarousel({items:4,responsive:{0:{items:it0,nav:navValue0},600:{items:it600,nav:navValue600},992:{items:it992,nav:navValue992,loop:!0},1200:{items:it1200,nav:navValue1200,loop:!0},1600:{items:it1600,nav:navValue1600,loop:!0}},loop:!0,margin:10,dots:!1,onInitialized:setTimeout(function(){$(".product_slider ").css('overflow','visible')},500),onTranslated:function(e){$("#featured").find(".active img").unveil(100,addAnimClassToImg)},navText:['<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M34.52 239.03L228.87 44.69c9.37-9.37 24.57-9.37 33.94 0l22.67 22.67c9.36 9.36 9.37 24.52.04 33.9L131.49 256l154.02 154.75c9.34 9.38 9.32 24.54-.04 33.9l-22.67 22.67c-9.37 9.37-24.57 9.37-33.94 0L34.52 272.97c-9.37-9.37-9.37-24.57 0-33.94z"></path></svg>','<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"></path></svg>'],slideSpeed:200,})}
jQuery(document).ready(function(){make_featured_slider()});function make_specials_slider(){var sliderObject=document.getElementById("specials");var itemsCount=$(">div",sliderObject).length;var it0=2;var it600=2;var it992=6;var it1200=4;var it1600=6;var navValue0=it0<itemsCount;var navValue600=it600<itemsCount;var navValue992=it992<itemsCount;var navValue1200=it1200<itemsCount;var navValue1600=it1600<itemsCount;$("#specials").owlCarousel({items:4,responsive:{0:{items:it0,nav:navValue0},600:{items:it600,nav:navValue600},992:{items:it992,nav:navValue992,loop:!0},1200:{items:it1200,nav:navValue1200,loop:!0},1600:{items:it1600,nav:navValue1600,loop:!0}},loop:!0,margin:10,dots:!1,onInitialized:setTimeout(function(){$(".product_slider ").css('overflow','visible')},500),onTranslated:function(e){$("#specials").find(".active img").unveil(100,addAnimClassToImg)},navText:['<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M34.52 239.03L228.87 44.69c9.37-9.37 24.57-9.37 33.94 0l22.67 22.67c9.36 9.36 9.37 24.52.04 33.9L131.49 256l154.02 154.75c9.34 9.38 9.32 24.54-.04 33.9l-22.67 22.67c-9.37 9.37-24.57 9.37-33.94 0L34.52 272.97c-9.37-9.37-9.37-24.57 0-33.94z"></path></svg>','<svg role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"></path></svg>'],slideSpeed:200,})}
jQuery(document).ready(function(){make_specials_slider()});if(cPath==""){gtag('event','page_view',{'send_to':'',"ecomm_pagetype":"home",})};$(document).ready(function(){doHookie('page_view');if(page_name=='checkout'){$('#pageContentContainer').show();onePage.initCheckout()}setTimeout(function(){$.ajaxSetup({cache:!0});$(".youtube_iframe").each(function(){$(this).attr("src",$(this).attr("data-src"))})},4000)})