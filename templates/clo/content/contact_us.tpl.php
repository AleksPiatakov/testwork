...
echo tep_draw_form(
    'contact_us',
    tep_href_link(FILENAME_CONTACT_US, tep_get_all_get_params(['action', 'language']) . 'action=send')
); ?>
<div class="container">
    <div class="row">
        ...
        <div class="col-xs-12 col-md-6">
            <div class="row">
                <div class="col-xs-12 col-sm-4">
                    <?= renderArticle('contacts_footer'); // contacts footer   ?>
                </div>
                <div class="col-xs-12 col-sm-7 form-group">
                    <input type="text" name="name" class="form-group form-control" placeholder="<?= ENTRY_NAME ?>"
                           required value="<?= $val_name ?>"/>
                    <input type="text" name="email"
                           class="form-group form-control"
                           placeholder="<?= ENTRY_EMAIL ?>" required
                           value="<?= $val_email ?>"/>
                    <input type="text"
                           name="phone"
                           class="form-group form-control"
                           placeholder="<?= ENTRY_PHONE ?>"
                           required/>
        ...
        <div class="col-xs-12 col-md-6 form-group">
            <?= renderArticle('google_map'); // google map   ?>
        </div>
    </div>
</div>
</form>
