...
if ($sidebar_left) {
    ...
    $center_classes .= 'col-12 col-sm-12 col-md-9';
} else {
    $center_margin = '';
    $center_classes .= 'col-12 padd-0';
}
...
if ($sidebar_left || isset($_GET['products_id']) || $content == 'checkout' || $content == 'compare' || $content == 'wishlist' || (isset($tPath) && !empty($tPath))) {
    $template_container_begin .= '<div class="' . ($template->getMainconf(
        'CONTENT_WIDTH'
    ) ? 'container' : 'container-fluid') . '"><div class="row">';
...
if (!$sidebar_left && ($_GET['articles_id'] || $_GET['tPath'])) {
    $breadcrumb_container_begin .= '<div class="' . ($template->getMainconf(
        'CONTENT_WIDTH'
...
$assets->cssInline[] = ':root {
          ...
          .product {height:' . (SMALL_IMAGE_HEIGHT + 154) . 'px;}
          .product_slider {height:' . (SMALL_IMAGE_HEIGHT + 160) . 'px;}
...
$assets->js[] = 'includes/javascript/superfish/superfish.js'; // <!-- УДАЛИТЬ ПОСЛЕ СМЕНЫ КЛАССА !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-->
