        ...
        <div class="filter_box_in">
            ...
            if ($config['manufacturers']['val'] == 1 && !empty($filterManufacturers) && empty($_GET['manufacturers_id'])) {
                ...
                foreach ($filterManufacturers as $manufacturers_values) {
                    ...
                    $manufacturersFilterHtml .= '<input type="checkbox"' . $disabledInput . ' id="brand_' . $manufacturers_values['manufacturers_id'] . '" name="filter_id[]" ' . $manufacturers_values['check'] . ' value="' . $manufacturers_values['manufacturers_id'] . '" />
                    ...
                    foreach ($attrs_array as $at_id) {
                        if (is_array($show_in_filter) and in_array($at_id, $show_in_filter)) {
                            ...
                            if ($checkFilterNext || $checkFilterRobots || isset($_GET['manufacturers_id']) || $allchecked) {
                                ...
                                $filterText = '<a href="' . getFilterUrl($_GET['cPath'], $_GET['filter_id'], $allOptionsVal) . '">' . FILTER_ALL . '</a>';
                            }
                            // output checkbox "all"
