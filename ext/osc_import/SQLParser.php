...
class SQLParser
{
    ...
    function walk($tokens, $sql, $source_map)
    {
        ...
        foreach ($statements as $stmt) {
            ...
            if ($this->find_single_table && count($tables)) {
                return array(
                'tables' => $tables,
                );
            }
    ...
    function parse_create_table($tokens, $i, $num)
    {
        ...
        if ($i <= count($tokens)) {
            $table['more'] = array_slice($tokens, $i);
        }
    ...
    function parse_field_or_key(&$tokens, &$fields, &$indexes)
    {
        ...
        switch ($tokens[0]) {
            ...
            case 'UNIQUE KEY':
                ...
                if ($has_constraint) {
                    ...
                    if (!is_null($constraint)) {
                        $index['constraint_name'] = $constraint;
                    }
                }
                ...
                if ($tokens[0] == 'UNIQUE INDEX') {
                    $index['type'] = 'UNIQUE';
                }
                if ($tokens[0] == 'UNIQUE KEY') {
                    $index['type'] = 'UNIQUE';
                }
            ...
            case 'SPATIAL KEY':
                ...
                if ($tokens[0] == 'SPATIAL INDEX') {
                    $index['type'] = 'SPATIAL';
                }
                if ($tokens[0] == 'SPATIAL KEY') {
                    $index['type'] = 'SPATIAL';
                }
            ...
            case 'FOREIGN KEY':
                ...
                if ($tokens[0] == 'REFERENCES') {
                    ...
                    if (count($tokens) >= 1 && $tokens[0] == 'MATCH FULL') {
                        $index['ref_match'] = 'FULL'   ;
                        array_shift($tokens);
                    }
                    if (count($tokens) >= 1 && $tokens[0] == 'MATCH PARTIAL') {
                        $index['ref_match'] = 'PARTIAL';
                        array_shift($tokens);
                    }
                    if (count($tokens) >= 1 && $tokens[0] == 'MATCH SIMPLE') {
                        $index['ref_match'] = 'SIMPLE' ;
                        array_shift($tokens);
                    }

                    if (count($tokens) > 1 && $tokens[0] == 'ON DELETE') {
                        array_shift($tokens);
                        $index['ref_on_delete'] = array_shift($tokens);
                    }
                    if (count($tokens) > 1 && $tokens[0] == 'ON UPDATE') {
                        array_shift($tokens);
                        $index['ref_on_delete'] = array_shift($tokens);
                    }
                }
