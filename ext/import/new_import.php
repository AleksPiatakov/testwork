...
if (EP_HTC_SUPPORT == true) {
    $default_these[] = 'metaTitle';
    $default_these[] = 'metaDesc';
}
...
if (!empty($_GET['download']) && ($_GET['download'] == 'stream' or $_GET['download'] == 'activestream' or $_GET['download'] == 'tempfile')) {
    ...
    foreach ($filelayout_header as $key => $value) {
        $filestring .= stripslashes($key) . $ep_separator;
    }
    ...
    while ($row) {
        ...
        $row[EASY_R_NAME] = stripslashes($row2['products_name']);
        ...
        if (isset($filelayout['metaTitle'])) {
            $row['metaTitle'] = $row2['products_head_title_tag'];
            $row['metaDesc'] = $row2['products_head_desc_tag'];
        }
...
?>
    ...
    <form name="clear_form" action="<?= $_SERVER['PHP_SELF'] . '?' . http_build_query($formParams) ?>" method="post"
          id="clear_form">
    ...
    <form name="clear_form2" action="<?= $_SERVER['PHP_SELF'] . '?' . http_build_query($formParams) ?>" method="post"
          id="clear_form2">
    ...
    //*******************************
        ...
        <form name="clear_form2" action="<?= $_SERVER['PHP_SELF'] . '?' . http_build_query($formParams) ?>"
              method="post" id="clear_form2">
            <input type="submit" name="clear_button2" value="Clear" style="padding: 0"/>
        </form>
            <?php
            echo "<p class=smallText>";
    ...
    <?php if (isset($_GET['action_type']) && $_GET['action_type'] == 'import') {?>
        <?php $formParams = [
            'split' => 0,
...
?>
        <form enctype="multipart/form-data"
              action="<?= $_SERVER['PHP_SELF'] . '?' . http_build_query($formParams) ?>"
              method="post"><?php if (defined('SID') && tep_not_null(SID)) {
            ...
            <div class="settings_block">
                <span class="w-50">
                    <?php echo EASY_LABEL_DELIMITER; ?>
                    <select class="form-control" name="separators">
                        <option value="1">;</option>
                        <option value="2" <?php echo($_SESSION['languages_code'] == 'en' ? 'selected' : '') ?> >,
                        </option>
                    </select>
                </span>
                <span class="w-50">
                    <?=mb_ucfirst(EASY_R_LANGUAGE); ?>
                    <?=tep_draw_pull_down_menu('epcust_language', $languages_array, $languages_code); ?>
                </span>
                <label>
                    <input type="checkbox" name="checkArttributes" id="checkArttributes">
                    <span><?php echo EASY_LABEL_IMPORT_FULL_ATTR_INF; ?></span>
                </label>
            </div>
            <div class="actions">
                <label>
                    <input type="file" class="load_file" name="usrfl" size="50" accept=".csv,
                    ...
                    <span id="replacement_block">
                            ...
                            <?php echo EASY_SELECT_FILE; ?>
                        </span>
                </label>
                <input type="submit" style="float: right;" class="green_btn" name="buttoninsert" value="<?php echo EASY_INSERT; ?>">
            </div>
            <br/>
            </p>
        </form>
    <?php } elseif (isset($_GET['action_type']) && $_GET['action_type'] == 'export') { ?>
        <p style="margin-top: 0;"><!-- Download file links -  Add your custom fields here -->
        <?php $formParams = ['import_type' => $_GET['import_type']]; ?>
        <form name="custom" action="<?= $_SERVER['PHP_SELF'] . '?' . http_build_query($formParams) ?>" method="get"
              id="custom">
            ...
            $manufacturers_array = array();
            $manufacturers_array[] = array("id" => '', 'text' => EASY_R_ALL);
            ...
            $status_array = array(array("id" => '', 'text' => EASY_R_ALL), array("id" => '1', 'text' => EASY_R_STATUS_ACT), array("id" => '0', 'text' => EASY_R_STATUS_NOACT));
            ...
            <div class="settings_block">
                <span class="w-30">
                    <?=EASY_EXPORT_DATA; ?>
                    <?=tep_draw_pull_down_menu('dltype', array(0 => array("id" => 'full', 'text' => EASY_R_ALL), 1 => array("id" => 'priceqty', 'text' => EASY_R_PRICEQTY)), 'custom', 'onChange="return switchForm(this);"'); ?>
                </span>
                <span class="w-30">
                    <?=mb_ucfirst(EASY_R_CAT); ?>
                    <?=tep_draw_pull_down_menu('epcust_category_filter', array_merge(array(0 => array("id" => '', 'text' => EASY_R_ALL)), $tep_get_category_tree ?: [])); ?>
                </span>
                <span class="w-30">
                    <?=mb_ucfirst(EASY_R_LANGUAGE); ?>
                    <?=tep_draw_pull_down_menu('epcust_language', $languages_array, $languages_code); ?>
                ...
                <span class="w-30">
                    <?=mb_ucfirst(EASY_R_MANUF); ?>
                    <?=tep_draw_pull_down_menu('epcust_manufacturer_filter', $manufacturers_array); ?>
                </span>
                <span class="w-30">
                    <?=mb_ucfirst(EASY_R_STATUS); ?>
                    <?=tep_draw_pull_down_menu('epcust_status_filter', $status_array); ?>
                </span>
                <span class="w-30">
                    <?=EASY_LABEL_DELIMITER; ?>
                    <?=tep_draw_pull_down_menu('separators', $separators_array, $default_separator_id); ?>
                </span>
                <label>
                    <input type="checkbox" name="checkArttributes" id="checkArttributes">
                    <span><?php echo EASY_LABEL_EXPORT_FULL_ATTR_INF; ?></span>
                </label>
            </div>
            <div class="actions">
                <input type='submit' class='btn button-small const-btn green_btn' value='<?=EASY_LABEL_PRODUCT_START; ?>'>
            </div>
        </form>
...
function ep_create_filelayout($dltype, $attribute_options_array, $languages, $custom_fields)
{
    ...
    if (EP_HTC_SUPPORT == true) {
        $ep_additional_layout_product_description .= '$filelayout[\'metaTitle\']    = $iii++;
                                                    $filelayout[\'metaDesc\']     = $iii++;
                                                    ';
    }
...
function process_row($item1, $filelayout, $filelayout_count, $default_these, $ep_separator, $languages, $custom_fields, $leng, $checkArttributes)
{
    ...
    $v_products_model = tep_db_input($items[$filelayout[EASY_R_MODEL]]);
...
//        $v_products_id    =    $items[$filelayout['v_products_id']];
        ...
        // Get the ID
        $sql = "SELECT p.products_id as v_products_id    FROM " . TABLE_PRODUCTS . " as p WHERE p.products_model = '" . tep_db_input($items[$filelayout[EASY_R_MODEL]]) . "'";
        $result = tep_db_query($sql);
        $row = tep_db_fetch_array($result);
        ...
        $sql = "SELECT
                ...
                FROM
                    ...
                    " . TABLE_PRODUCTS_TO_CATEGORIES . " as ptoc
                WHERE
                    p.products_model = '" . tep_db_input($items[$filelayout[EASY_R_MODEL]]) . "' AND
                    p.products_id = ptoc.products_id AND
                    ptoc.categories_id = subc.categories_id
                LIMIT 1
            ";
        ...
        /**
         * The logic of import has changed
         * If there is a product code in the database then update, if not then add,
         * if the code field is empty then skip it, if the status of the product is deleted then deleted
         */
        ...
        if ($process_product == true) {
                ...
                for ($categorylevel = EP_MAX_CATEGORIES + 1; $categorylevel > 0; $categorylevel--) {
                    ...
                    if (!empty($thiscategoryname)) {
                        ...
                        if (!empty($row['categories_id'])) {
                            ...
                            $result = tep_db_query($sql);
                            $max_category_id = tep_db_insert_id();
                            ...
                            foreach ($languages as $key => $lang) {
                                $sql = "INSERT INTO " . TABLE_CATEGORIES_DESCRIPTION . " (
                                                ...
                                                $max_category_id,
                                                '" . $lang['id'] . "',
                                                '" . (!empty($v_categories_name[$categorylevel][$lang['id']]) ? trim(tep_db_input($v_categories_name[$categorylevel][$lang['id']])) : '') . "'
                                       )";
                                tep_db_query($sql);
                            }
            ...
            if (!empty($v_products_model)) {
                ...
                if (isset($filelayout[EASY_R_QTY])) {
                    ...
                    $v_products_quantity = (int) $v_products_quantity;
...
//                                    }
                ...
                if (isset($filelayout[EASY_R_NAME]) || isset($filelayout[EASY_R_DESC]) || $doit == true) {
                    ...
                    // header tags controller support
                    if (isset($filelayout['metatitle'])) {
                        $head_title_tag_var = 'metatitle';
                        $head_desc_tag_var = 'metadesc';
                    ...
                    if (tep_db_num_rows($result) > 0) {
                        $sql = "UPDATE " . TABLE_PRODUCTS_DESCRIPTION . "
                                   SET products_name='" . tep_db_input($$products_var) . "',
                                       $ep_additional_updates
                ...
                if (isset($v_customer_price_1)) {
                    if (($v_customer_group_id_1 == '') and !empty($v_customer_price_1)) {
                ...
                if (tep_db_num_rows($attribute_options_values7) > 0) {
                    ...
                    foreach ($attribute_options_array7 as $attribute_options) {
                        if (isset($filelayout[mb_strtolower($attribute_options['products_options_name'])])) {
                    ...
                    foreach ($attribute_options_array7 as $attribute_options) {
                        if (isset($filelayout[mb_strtolower($attribute_options['products_options_name'])])) {
                            if (!empty($items[$filelayout[mb_strtolower($attribute_options['products_options_name'])]])) {
                                if ($checkArttributes) {
                                    $pieces = explode("||", $items[$filelayout[mb_strtolower($attribute_options['products_options_name'])]]);
...
//                                            $id['max']++;
                                            ...
                                            $attribute_values_new_query = "insert into " . TABLE_PRODUCTS_OPTIONS_VALUES . " (language_id, products_options_values_name) values (" . $leng . ", '" . tep_db_input($tmpAttr[$i][0], DB()) . "')";
                                            $attribute_values_new = tep_db_query($attribute_values_new_query);
                                            $id['max'] = tep_db_insert_id();
                                    ...
                                    for ($i = 0; $i < $InputQuantityAttr; $i++) {
                                        ...
                                        if (tep_db_num_rows($attribute_values_query) <= 0) {
                                            ...
                                            $attribute_values_new_query = "insert into " . TABLE_PRODUCTS_OPTIONS_VALUES . " (language_id, products_options_values_name) values (" . $leng . ", '" . tep_db_input($pieces[$i], DB()) . "')";
                                            $attribute_values_new = tep_db_query($attribute_values_new_query);
                                            $id['max'] = tep_db_insert_id();
