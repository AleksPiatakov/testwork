...
function ship2pay()
{
    ...
    $q_ship2pay = tep_db_query(
        "SELECT payments_allowed, zones_id FROM " .
        TABLE_SHIP2PAY . " where shipment = '" .
        $shipping_module . "' and status = 1"
    );
    $check_flag = false;
    while ($mods = tep_db_fetch_array($q_ship2pay)) {
        if ($mods['zones_id'] > 0) {
            $check_query = tep_db_query(
                ...
                " where geo_zone_id = " . (int)$mods['zones_id'] .
