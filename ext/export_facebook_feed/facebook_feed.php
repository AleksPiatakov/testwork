...
class GoogleMerchant
{
    ...
    {
        global $languages_id;
        $this->product_image_path = GoogleMerchant::SHOP_LINK . "/getimage/products/";
    ...
    public function productsQuery()
    {
        ...
        `mi`.`manufacturers_name`, `s`.`specials_new_products_price`, `s`.`specials_date_added`, `s`.`expires_date`, `p2c`.`categories_id`
        FROM `products` `p` 
    ...
    public function getProducts()
    {
        foreach ($this->productsRaw as $row) {
            ...
            if ($row['specials_new_products_price']) {
                ...
                $sale_end = strtotime($row['expires_date']);
                ...
                //                $sale_start = date('Y-m-d\TH:i:s',strtotime($row['specials_date_added']));
                //                $sale_end = date('Y-m-d\TH:i:s',strtotime($row['expires_date']));
                //                $product['sale_price_effective_date'] =  $sale_start. "/" . $sale_end; //YYYY-MM-DD or YYYY-MM-DDThh:mm:ss or YYYY-MM-DDThh:mm:ssZ
            }
