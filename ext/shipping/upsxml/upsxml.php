...
class upsxml
{
    ...
    function __construct()
    {
        ...
        $this->sort_order = getConstantValue('MODULE_SHIPPING_UPSXML_SORT_ORDER', '0');
        $this->icon = DIR_WS_ICONS . 'shipping_ups.png';
...
// we will set the default to pounds (LBS) and inches (IN)
      ...
      //  $this->logfile = DIR_FS_CATALOG.'/upsxml.log';
...
//        $this->ups_error_file = '/srv/www/htdocs/catalog/includes/modules/shipping/upsxml_error.log';
        ...
        if (($this->enabled == true) && ((int)MODULE_SHIPPING_UPSXML_RATES_ZONE > 0) and !$admin_check) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)MODULE_SHIPPING_UPSXML_RATES_ZONE . "' and zone_country_id = '" . (int)$order->delivery['country']['id'] . "' order by zone_id");
...
// of insurance if not divided by the number of boxes
      ...
      // EOF Time In Transit
        ...
        if ((is_array($upsQuote)) && (sizeof($upsQuote) > 0)) {
            ...
            for ($i = 0; $i < sizeof($upsQuote); $i++) {
                foreach ($upsQuote[$i] as $type => $cost) {
                    ...
                    if ($this->exclude_choices($basetype)) {
                        continue;
                    }
                    // EOF limit choices
                    if ($method == '' || $method == $basetype) {
                        ...
                        if ($this->handling_type == "Percentage") {
                            if ($_type) {
                                $methods[] = array('id' => $basetype, 'title' => $_type, 'cost' => ((($this->handling_fee * $cost) / 100) + $cost));
                            }
                        } else {
                            if ($_type) {
                                $methods[] = array('id' => $basetype, 'title' => $_type, 'cost' => ($this->handling_fee + $cost));
                            }
                        }
                    }
                }
    ...
    {
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable UPS Shipping', 'MODULE_SHIPPING_UPSXML_RATES_STATUS', 'true', 'Do you want to offer UPS shipping?', '6', '0', 'tep_cfg_select_option_checkbox(array(\'true\', \'false\'), ', now())");
...
// MODULE_SHIPPING_UPSXML_DIMENSIONS_SUPPORT -> SHIPPING_DIMENSIONS_SUPPORT
       ...
       // next three keys added to be able to use negotiated rates (available from UPS since about July 2006)
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order of display.', 'MODULE_SHIPPING_UPSXML_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '19', now())");
        // add key for disallowed shipping methods
        tep_db_query("insert into " . TABLE_CONFIGURATION . " ( configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Disallowed Shipping Methods', 'MODULE_SHIPPING_UPSXML_TYPES', '', 'Select the UPS services <span style=\'color: red; font-weight: bold\'>not</span> to be offered.', '6', '20', 'upsxml->get_multioption_upsxml',  'upsxml_cfg_select_multioption_indexed(array(\'US_01\', \'US_02\', \'US_03\', \'US_07\', \'US_54\', \'US_08\', \'CAN_01\', \'US_11\', \'US_12\', \'US_13\', \'US_14\', \'CAN_02\', \'US_59\', \'US_65\', \'CAN_14\', \'MEX_54\', \'EU_82\', \'EU_83\', \'EU_84\', \'EU_85\', \'EU_86\'), ',  now())");
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Display Weight', 'MODULE_SHIPPING_UPSXML_WEIGHT1', 'true', 'Do you want to show number of packages and package weight?', '6', '28', 'tep_cfg_select_option_checkbox(array(\'true\', \'false\'), ', now())");
    }
...
/*        $filename = '/srv/www/htdocs/catalog/includes/modules/shipping/example_response.xml';
        ...
        $oldXmlResult = $xmlResult;
        $newXmlResult = $this->_parseResult($xmlResult);

        return !empty($newXmlResult) ? $newXmlResult : $oldXmlResult;
    }
    ...
    function _parseResult($xmlResult)
    {
        ...
        if ($responseStatusCode != '1') {
            $errorMsg = '';
            if (isset($doc['RatingServiceSelectionResponse']['Response']['Error'])) {
                foreach ($doc['RatingServiceSelectionResponse']['Response']['Error'] as $error) {
                    if (is_array($error)) {
                        $errorMsg .= $error['ErrorCode'] . ": " . $error['ErrorDescription'] . "<br />";
                    }
                }
            }
            if (isset($doc['RatingServiceSelectionResponse']['Response']['ResponseStatusDescription'])) {
                if (is_array($error)) {
                    $errorMsg .= $error['ErrorCode'] . ": " . $doc['RatingServiceSelectionResponse']['Response']['ResponseStatusDescription'] . "<br />";
                }
            }


            // send email if enabled in the admin section
            if ($this->email_errors && !empty($this->access_key)) {
                error_log("UPSXML Rates Error: " . $errorMsg . " server =  " . $_SERVER['SERVER_NAME'] . "experienced by customer with id " . $_SESSION['customer_id'] . " on " . date('Y-m-d H:i:s'), 1, STORE_OWNER_EMAIL_ADDRESS);
            }
            // log errors to file ups_error.log when set
