...
class usps
{
    ...
    {
        global $order, $packing,$admin_check;
        $this->code = 'usps';
        $this->version = '7.3.1';
        ...
        if (($this->enabled == true) && ((int)MODULE_SHIPPING_USPS_ZONE > 0) and !$admin_check) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)MODULE_SHIPPING_USPS_ZONE . "' and zone_country_id = '" . (int)$order->delivery['country']['id'] . "' order by zone_id");
    ...
    // solomono: hide USPS for US customers:
//    if($order->delivery['country']['id']==223) $this->enabled = false;
        ...
        $this->sort_order = getConstantValue('MODULE_SHIPPING_USPS_SORT_ORDER', '0');
    ...
    function quote($method = '')
    {
        ...
        if (getConstantValue('SHIPPING_UNIT_WEIGHT', '') == 'KGS') { // must convert kilograms to pounds before getting quote
            for ($i = 0; $i < sizeof($boxesToShip); $i++) {
        ...
        if ($order->delivery['country']['id'] == SHIPPING_ORIGIN_COUNTRY) { // domestic quote
            ...
            if (!$error) {
                ...
                foreach ($dmstcquotes as $quote) {
                    if ($quote['count'] != $numBoxes) {
                        continue; // skip methods that don't work for all packages
                    }
                    if ((MODULE_SHIPPING_USPS_DMSTC_RATE == 'Internet') && $quote['onlineavail']) {
                        $title = $quote['name']; //$this->types[$quote['id']];
                        if ($this->display_insurance && ($quote['onlineinsval'] > 0)) {
                            $title .= '<span class="usps_desc">' . MODULE_SHIPPING_USPS_TEXT_INSURED . '$' . (ceil($quote['onlineinsval'] * 100) / 100) . '</span>';
                        }
                        if ($this->display_confirmation && tep_not_null($quote['onlineconf'])) {
                            $title .= '<span  class="usps_desc">' . $quote['onlineconf'] . '</span>';
                        }
                        if ($this->display_transit && ($trnstime !== false)) {
                            ...
                            if (!empty($time)) {
                                $title .= '<span class="usps_desc">' . $time . '</span>';
                            }
                        }
                        ...
                        $title = $quote['name']; //$this->types[$quote['id']];
                        if ($this->display_insurance && ($quote['retailinsval'] > 0)) {
                            $title .= '<span class="usps_desc">' . MODULE_SHIPPING_USPS_TEXT_INSURED . '$' . (ceil($quote['retailinsval'] * 100) / 100) . '</span>';
                        }
                        if ($this->display_confirmation && tep_not_null($quote['retailconf'])) {
                            $title .= '<span class="usps_desc">' . $quote['retailconf'] . '</span>';
                        }
                        if ($this->display_transit && ($trnstime !== false)) {
                            $time = '';
                            if ((strpos($quote['id'], 'First') !== false) || (strpos($quote['id'], 'Priority') !== false)) {
                ...
                foreach ($intlquotes as $quote) {
                    ...
                    if ((MODULE_SHIPPING_USPS_INTL_RATE == 'Internet') && $quote['onlineavail']) {
                        ...
                        if ($this->display_transit && tep_not_null($quote['transtime'])) {
                            $title .= '<span class="usps_desc">' . MODULE_SHIPPING_USPS_TEXT_ESTIMATED . $quote['transtime'] . '</span>';
                        }
                        if (MODULE_SHIPPING_USPS_HANDLING_TYPE == 'Per Shipment') {
                        ...
                        if (MODULE_SHIPPING_USPS_HANDLING_TYPE == 'Per Shipment') {
                            ...
                            if (strpos($this->intl_handling[$quote['id']], '%') !== false) {
                                $cost = $quote['retailrate'] * ((100 + (int)$this->intl_handling[$quote['id']]) / 100 * $numBoxes);
                            } else {
...
//      print_r( $this->quotes );
    ...
    static function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable USPS Shipping', 'MODULE_SHIPPING_USPS_STATUS', 'true', 'Do you want to offer USPS shipping?', '6', '0', 'tep_cfg_select_option_checkbox(array(\'true\', \'false\'), ', now())");
        ...
        $cfglist = array();
        $handling = array();
        foreach (static::$_types_ as $title) {
            $cfglist[] = "\'" . $title . "\'";
            $handling[] = '0';
        }
        // Values to select domestic shipping methods and handling are set from list created in function usps
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Domestic Shipping Methods', 'MODULE_SHIPPING_USPS_DMSTC_TYPES', '" . implode(', ', static::$_types_) . "', 'Select the domestic services to be offered:', '6', '0', 'usps_cfg_select_multioption(array(" . implode(', ', $cfglist) . "), ', now())");
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Domestic Insurance', 'MODULE_SHIPPING_USPS_DMSTC_INSURANCE_OPTION', 'true', 'Use USPS Calculated Domestic Insurance?', '6', '0', 'tep_cfg_select_option_checkbox(array(\'true\', \'false\'), ', now())");
    ...
    function _getDmstcQuote($pkg, $method = '')
    {
        ...
        if ($pkgvalue > MODULE_SHIPPING_USPS_DMSTC_INSURANCE_MAX) {
            $pkgvalue = MODULE_SHIPPING_USPS_DMSTC_INSURANCE_MAX;
        }
        if ($pkgvalue <= 0) {
            $pkgvalue = 0.01;
        }
        ...
        if (MODULE_SHIPPING_USPS_DMSTC_INSURANCE_OPTION == 'true') {
            $sservs = array(1, 11); //request insurance, both regular & express mail
        }
        ...
        $sservice = '';
        if (!empty($sservs)) {
            foreach ($sservs as $id) {
                $sservice .= '<SpecialService>' . $id . '</SpecialService>';
            }
        ...
        foreach ($request_types as $service) { // break the requested shipping methods into groups
            if (strpos($service, 'First') !== false) { // if first class mail service check if will fit type
                ...
                if (($fcmt == 'LETTER') && (($length > $this->first_class_letter['length']) || ($width > $this->first_class_letter['width']) || ($height > $this->first_class_letter['height']) || ($ounces > $this->first_class_letter['ounces']))) {
                    continue; // don't check letter type if package doesn't fit
                }
                if (($fcmt == 'FLAT') && (($length > $this->first_class_large_letter['length']) || ($width > $this->first_class_large_letter['width']) || ($height > $this->first_class_large_letter['height']) || ($ounces > $this->first_class_large_letter['ounces']))) {
                    continue;  // don't check large letter type if package doesn't fit
                }
                if (($fcmt == 'POSTCARD') && (($length > 6) || ($width > 4.25) || ($height > 0.016) || ($ounces > 3.5))) {
                    continue;  // don't check postcard type if package doesn't fit
                }
                if ($ounces > $this->first_class_parcel_max_ounces) {
                    continue; // don't check First Class if too heavy
                }
            }
            ...
            if (strpos($cont, 'Envelope') !== false) { // if service container is envelope
                if ($height > 1) {
                    continue; // anything thicker than one inch won't fit an envelope
                }
                if ((strpos($cont, 'SM') !== false) && (($length > 10) || ($width > 6))) {
                    continue;
                }
                if ((strpos($cont, 'Window') !== false) && (($length > 10) || ($width > 5))) {
                    continue;
                }
                if ((strpos($cont, 'Gift') !== false) && (($length > 10) || ($width > 7))) {
                    continue;
                }
                if ((strpos($cont, 'Legal') !== false) && (($length > 15) || ($width > 9.5))) {
                    continue;
                }
                if (($length > 12.5) || ($width > 9.5)) {
            ...
            if (($cont == 'LG Flat Rate Box') || ($cont == 'Flat Rate Box')) {
                if ($length > $DT[10]) {
                    continue; // too big for longest medium box
                }
                if ($length > $DT[13]) { // check medium type 2 box
                    if (
                        ...
                        continue; // won't fit medium type 2 box
                    }
                } elseif (
                    ($length > $DT[13])
                        ||
                    ...
                    continue; // won't fit either type medium box
                }
            }

            if (
                ($cont == 'MD Flat Rate Box')
                &&
                (
                ($length > $DT[4])
                    ||
            ...
            if ($cont == 'Regional Rate Box A') {
                if ($length > 12.8125) {
                    continue; // too big for longest A box
                }
                if ($length > 10) { // check A2 box
                    if (($length > 12.8125) || ($width > 10.9375) || ($height > 2.375)) {
                        continue; // won't fit A2 box
                    }
                } elseif (($length > 10) || ($width > 7) || ($height > 4.75)) {
                    continue; // won't fit either type A box
                }
            }
            if ($cont == 'Regional Rate Box B') {
                if ($length > 15.875) {
                    continue; // too big for longest B box
                }
                if ($length > 12) { // check B2 box
                    if (($length > 15.875) || ($width > 14.375) || ($height > 2.875)) {
                        continue; // won't fit B2 box
                    }
                } elseif (($length > 12) || ($width > 10.25) || ($height > 5)) {
                    continue; // won't fit either type B box
                }
            }
            if (($cont == 'Regional Rate Box C') && (($length > 14.75) || ($width > 11.75) || ($height > 11.5))) {
                continue;
            }
            // passed all size checks so build request
            $request = '<Package ID="' . $id . '"><Service>' . $this->type_to_request[$service] . '</Service>';
            if (strpos($service, 'First') !== false) {
                $request .= '<FirstClassMailType>' . $fcmt . '</FirstClassMailType>';
            }
            ...
            if (MODULE_SHIPPING_USPS_DMSTC_INSURANCE_OPTION == 'true') {
                $request .= '<Value>' . number_format($pkgvalue, 2, '.', '') . '</Value>';
            }
            $request .= $sservice;
            if ((strpos($service, 'First') !== false) || (strpos($service, 'Post') !== false)) {
    ...
    //  debug($request); exit;
        ...
        if (empty($request_group)) {
            return array('error' => MODULE_SHIPPING_USPS_ERROR_NO_SERVICES);
        }
        if (!class_exists('httpClient')) {
            include(DIR_WS_CLASSES . 'http_client.php');
    ...
    function _decode_domestic_response($service, $pkgvalue, $nonuspsinsurancecost, $opval)
    {
        ...
        if (MODULE_SHIPPING_USPS_DMSTC_INSURANCE_OPTION == 'true') {
            $onlineavailable = ($onlineavailable && ($pkgvalue <= MODULE_SHIPPING_USPS_DMSTC_INSURANCE_MAX_ONLINE)); // can't use online if package value exceeds maximum available online
        }
    ...
    function _getDmstcTransitTimes()
    {
        ...
        if ($http->Connect($usps_server, 80)) {
            ...
            $http->addHeader('User-Agent', 'osCommerce');
            $http->addHeader('Connection', 'Close');
            if ($http->Get('/' . $api_dll . '?' . $transitpriorityreq)) {
                $transpriorityresp = $http->getBody();
            }
            if ($http->Get('/' . $api_dll . '?' . $transitexpressreq)) {
                $transexpressresp = $http->getBody();
            }
            if ($http->Get('/' . $api_dll . '?' . $transitparcelreq)) {
                $transparcelresp = $http->getBody();
            }
            $http->Disconnect();
        } else {
            return false;
...
//      exit;
        ...
        if (isset($doc['IntlRateV2Response']['Package']['Error'])) {
            return array('error' => $doc['IntlRateV2Response']['Package']['Error']['Number'] . ' ' . $doc['IntlRateV2Response']['Package']['Error']['Description']);
        }
        if (isset($doc['IntlRateV2Response']['Package'][0]['Error'])) {
            return array('error' => $doc['IntlRateV2Response']['Package']['Error'][0]['Number'] . ' ' . $doc['IntlRateV2Response']['Package'][0]['Error']['Description']);
        }
        if (isset($doc['IntlRateV2Response']['Package']['Service']['SvcDescription'])) { // single mail service response
            ...
            foreach ($doc['IntlRateV2Response']['Package'] as $package) {
                if (isset($package['Service']['SvcDescription'])) { // single mail service response for package
                    ...
                    if (!empty($tmp)) {
                        if (!isset($quotes[$tmp['id']])) {
                            $quotes[$tmp['id']] = $tmp; // save only first valid response
                        }
                    }
    ...
    function _decode_intl_response($service, $nonuspsinsurancecost, $opval)
    {
        ...
        if (MODULE_SHIPPING_USPS_INTL_INSURANCE_OPTION == 'true') {
            if (isset($service['InsComment'])) {
                if ($service['InsComment'] == 'INSURED VALUE') {
                    return array(); // skip if invalid insured value when doing USPS insurance
                }
            } else {
        ...
        if (!in_array($qid, $this->intl_request_types)) {
            return array(); // not an allowed international method
        }
        ...
        $onlineavailable = ($onlinerate > 0);
        if (MODULE_SHIPPING_USPS_INTL_INSURANCE_OPTION == 'true') {
            $onlineavailable = $onlineavailable && ($insured_for <= MODULE_SHIPPING_USPS_INTL_INSURANCE_MAX_ONLINE); // can't use online for USPS international insurance over maximum
        }
...
//      echo "[{$service['SvcCommitments']}]<br />";
    ...
    private function updateTypeChangeStorage($params = array())
    {
        ...
        foreach ($existing_changes as $k => $change) {
            if (
                in_array($change, ($params['type'] === 'International'
                    ?
                $this->intl_types
                    :
                $this->types
