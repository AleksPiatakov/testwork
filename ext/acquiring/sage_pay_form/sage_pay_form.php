...
class sage_pay_form
{
    ...
    function __construct()
    {
        ...
        $this->public_title = MODULE_PAYMENT_SAGE_PAY_FORM_TEXT_PUBLIC_TITLE . (!isMobile() ? MODULE_PAYMENT_SAGE_PAY_FORM_IMAGES : '');
    ...
    function update_status()
    {
        ...
        if (
            $this->enabled == true &&
            (int)getConstantValue('MODULE_PAYMENT_SAGE_PAY_FORM_ZONE', 0) > 0 &&
            getConstantValue('ACCOUNT_COUNTRY') == 'true' && getConstantValue('ACCOUNT_STATE') == 'true'
        ) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)MODULE_PAYMENT_SAGE_PAY_FORM_ZONE . "' and zone_country_id = '" . (int)$order->customer['country']['id'] . "' order by zone_id");
            while ($check = tep_db_fetch_array($check_query)) {
                if ($check['zone_id'] < 1) {
                    $check_flag = true;
                    break;
                } elseif ($check['zone_id'] == $order->customer['zone_id']) {
                    $check_flag = true;
                    break;
                }
            }
    ...
    {
        global $customer_id, $order, $currency, $cartID, $cart_payment_id, $insert_id;
        ...
        $crypt = array(
            ...
            'VendorTxCode' => substr(date('YmdHis') . '-' . $customer_id . '-' . $order_id, 0, 40),
            'Amount' => $this->format_raw($order->info['total']),
            'Currency' => $currency,
            'Description' => substr(STORE_NAME, 0, 100),
            'SuccessURL' => addHostnameToLink(tep_href_link('includes/callbacks/sage_callback.php', '', 'SSL')),
            'FailureURL' => addHostnameToLink(tep_href_link(FILENAME_CHECKOUT, 'payment_error=' . $this->code, 'SSL')),
            ...
            'BillingSurname' => substr($order->customer['lastname'], 0, 20),
            'BillingFirstnames' => substr($order->customer['firstname'], 0, 20),
            'BillingAddress1' => substr($order->customer['street_address'], 0, 100),
            'BillingCity' => substr($order->customer['city'], 0, 40),
            'BillingPostCode' => substr($order->customer['postcode'], 0, 10),
            'BillingCountry' => $order->customer['country']['iso_code_2']
        );

        if ($crypt['BillingCountry'] == 'US') {
            $crypt['BillingState'] = tep_get_zone_code($order->customer['country']['id'], $order->customer['zone_id'], '');
        }
        ...
        foreach ($order->products as $product) {
            ...
            $contents[] = str_replace(array(':', "\n", "\r", '&'), '',
                    $product_name) . ':' . $product['qty'] . ':' . $this->format_raw($product['final_price']) . ':' . $this->format_raw(($product['tax'] / 100) * $product['final_price']) . ':' . $this->format_raw((($product['tax'] / 100) * $product['final_price']) + $product['final_price']) . ':' . $this->format_raw(((($product['tax'] / 100) * $product['final_price']) + $product['final_price']) * $product['qty']);
        }
        ...
        file_put_contents("__sage-form.log", var_export($crypt, true) . PHP_EOL, FILE_APPEND);
        $params['Crypt'] = $this->encryptParams($crypt_string);

        file_put_contents("__sage-form.log", var_export($crypt_string, true) . PHP_EOL, FILE_APPEND);
        file_put_contents("__sage-form.log", var_export($this->decryptParams($params['Crypt']), true) . PHP_EOL, FILE_APPEND);
    ...
    {
        global $_GET, $sage_pay_response, $callback, $cart_payment_id, $insert_id;
        ...
        if ($callback === true) {
            if (isset($_GET['crypt']) && tep_not_null($_GET['crypt'])) {
                ...
                $sage_response_id = $sage_pay_response['VendorTxCode'];
                $sage_response_id = explode("-", $sage_response_id);
                $sage_response_id = end($sage_response_id);

                file_put_contents(
                    "__sage_callback.log",
                    var_export($sage_pay_response, true) . PHP_EOL . PHP_EOL,
                    FILE_APPEND
                );
                ...
                if (($sage_pay_response['Status'] != 'OK') && ($sage_pay_response['Status'] != 'AUTHENTICATED') && ($sage_pay_response['Status'] != 'REGISTERED')) {
                    ...
                    tep_redirect(addHostnameToLink(tep_href_link(FILENAME_CHECKOUT, 'payment_error=' . $this->code . (tep_not_null($error) ? '&error=' . $error : ''), 'SSL')));
                }

                if ($sage_response_id != $order_id) {
                    $this->code = 1;
                    $error = "Invalid transaction order id";
                    tep_redirect(addHostnameToLink(tep_href_link(FILENAME_CHECKOUT, 'payment_error=' . $this->code . (tep_not_null($error) ? '&error=' . $error : ''), 'SSL')));
                }
            } else {
                tep_redirect(addHostnameToLink(tep_href_link(FILENAME_CHECKOUT, 'payment_error=' . $this->code, 'SSL')));
            }

            $_SESSION['allowCheckoutSuccessPageId'] = $order_id;
        }
    }

    function after_process($run = false)
    {

        if ($run) {
            global $insert_id, $sage_pay_response, $cart_payment_id, $onePageCheckout, $order;
            ...
            tep_db_query("
                UPDATE orders 
                SET orders_status = '" . MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_ORDER_STATUS_ID . "'
...
// don't show an error message for user cancelled/aborted transactions
    ...
    static function install($parameter = null)
    {
    ...
    {
        $keys = array_keys(self::getParams());
...
//        }
    ...
    static function getParams()
    {
        if (!defined('MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_ORDER_STATUS_ID')) {
            ...
            if (tep_db_num_rows($check_query) < 1) {
                ...
                if (tep_db_num_rows($flags_query) == 1) {
                    ...
                    tep_db_query("update " . TABLE_ORDERS_STATUS . " set public_flag = 0 where orders_status_id = '" . $status_id . "'");
                }
            } else {
        ...
        $params = array(
            'MODULE_PAYMENT_SAGE_PAY_FORM_STATUS' => array(
                'title' => 'Enable Sage Pay Module',
                'desc' => 'Do you want to accept Sage Pay payments?',
                'value' => 'True',
                'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '
            ),
            'MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME' => array(
                'title' => 'Vendor Login Name',
                'desc' => 'The vendor login name to connect to the gateway with.'
            ),
            'MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD' => array(
                'title' => 'Encryption Password',
                'desc' => 'The encrpytion password to secure and verify transactions with.'
            ),
            'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_METHOD' => array(
                'title' => 'Transaction Method',
                'desc' => 'The processing method to use for each transaction.',
                'value' => 'Authenticate',
                'set_func' => 'tep_cfg_select_option(array(\'Authenticate\', \'Deferred\', \'Payment\'), '
            ),
            'MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_EMAIL' => array(
            ...
            'MODULE_PAYMENT_SAGE_PAY_FORM_SEND_EMAIL' => array(
            ...
            'MODULE_PAYMENT_SAGE_PAY_FORM_ZONE' => array(
                'title' => 'Payment Zone',
            ...
            'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_SERVER' => array(
...
// format prices without currency formatting
    ...
    function sendDebugEmail($response = array())
    {
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                tep_mail('', MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL, 'Sage Pay Debug E-Mail', trim($email_body), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
            }
        }
    }
}
