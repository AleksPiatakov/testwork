...
// class constructor
    function __construct()
    {
        ...
        $this->title = MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TEXT_TITLE . (!isMobile() ? MODULE_PAYMENT_AUTHORIZENET_CC_AIM_IMAGES : '');
...
// class methods
    function update_status()
    {
        ...
        if (
            $this->enabled == true &&
            (int)getConstantValue('MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ZONE', 0) > 0 &&
            getConstantValue('ACCOUNT_COUNTRY') == 'true' && getConstantValue('ACCOUNT_STATE') == 'true'
        ) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ZONE . "' and zone_country_id = '" . (int)$order->billing['country']['id'] . "' order by zone_id");
        ...
        <div class="new_payment_form">
            <div id="payment-form">
                ...
                <label class="number_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_NUMBER .
                tep_draw_input_field('cc_number', isset($_SESSION['cc_number']) ? $_SESSION['cc_number'] : '',
                    'inputmode="numeric" onkeypress="validateCheckoutInput(event)" maxlength="16" class="form-control required checkout_inputs" placeholder="0000 0000 0000 0000"', 'tel') . '
                </label>
                <label class="cvc_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_CVC .
                tep_draw_password_field('cc_cvc_nh-dns', '', 'inputmode="numeric" onkeypress="validateCheckoutInput(event)" size="5" maxlength="4" class="form-control" placeholder="***"') . '
                </label>
                <label class="month_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_MM .
                tep_draw_input_field('cc_expires_month', '',
                    'inputmode="numeric" onkeypress="validateCheckoutInput(event)" maxlength="2" class="form-control required checkout_inputs" placeholder="' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_MM_PLACEHOLDER . '" ',
                    'tel') . '
                 </label>        
                <label class="year_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_YY .
                tep_draw_input_field('cc_expires_year', '',
                    'inputmode="numeric" onkeypress="validateCheckoutInput(event)" maxlength="2" class="form-control required checkout_inputs" placeholder="' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_YY_PLACEHOLDER . '" ',
                    'tel') . '
                </label>
            </div>
        </div> ';
        ...
        <div class="new_payment_form">
            <div id="payment-form">
                ...
                <label class="number_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_NUMBER .
                tep_draw_input_field('cc_number', isset($_SESSION['cc_number']) ? $_SESSION['cc_number'] : '',
                    'onkeypress="validateCheckoutInput(event)" maxlength="16" class="form-control required checkout_inputs" placeholder="0000 0000 0000 0000"', 'tel') . '
                </label>
                <label class="month_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_MM .
                tep_draw_input_field('cc_expires_month', '',
                    'onkeypress="validateCheckoutInput(event)" maxlength="2" class="form-control required checkout_inputs" placeholder="' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_MM_PLACEHOLDER . '" ',
                    'tel') . '
                 </label>        
                <label class="year_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_YY .
                tep_draw_input_field('cc_expires_year', '',
                    'onkeypress="validateCheckoutInput(event)" maxlength="2" class="form-control required checkout_inputs" placeholder="' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_EXPIRES_YY_PLACEHOLDER . '" ',
                    'tel') . '
                </label>
                <label class="cvc_field">
                    ' . MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_CVC .
                tep_draw_password_field('cc_cvc_nh-dns', '', 'onkeypress="validateCheckoutInput(event)" size="5" maxlength="4" class="form-control" placeholder="***"') . '
                </label>
            </div>
        </div> ';
        }
    ...
    function confirmation()
    {
        ...
        $confirmation = array(
            'fields' => array(
                array(
                    'title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_OWNER,
                ...
                array(
                    'title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CREDIT_CARD_CVC,
    ...
    function before_process()
    {
        ...
        $params = array(
            ...
            'x_amount' => substr($this->format_raw($total), 0, 15),
            ...
            'x_card_num' => substr($_POST['cc_number'], 0, 22),
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            $post_string .= '&x_line_item=' . urlencode($i + 1) . '<|>' . urlencode(substr($order->products[$i]['name'], 0, 31)) . '<|>' . urlencode(substr($order->products[$i]['name'], 0,
                    255)) . '<|>' . urlencode($order->products[$i]['qty']) . '<|>' . urlencode($this->format_raw($order->products[$i]['final_price'])) . '<|>' . urlencode($order->products[$i]['tax'] > 0 ? 'YES' : 'NO');
        }
        ...
        $apiLogin = MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID;
        $transId = $regs[6];
//        $amount=$this->format_raw($order->info['total']);
        $amount = $this->format_raw($total);
//        $signatureKey='4AABC35E5490F8A8FAE0C817EFC730908ECCA9CCC25C0B85809C0FF111EF09A92CE729314C82650DC7CC106BF12769FB660A9DBC83D8CC16F85C6A515086AF4E'; //https://support.authorize.net/s/article/What-is-a-Signature-Key
        $signatureKey = MODULE_PAYMENT_AUTHORIZENET_CC_AIM_SIGNATURE_KEY; //https://support.authorize.net/s/article/What-is-a-Signature-Key
        $textToHash = "^" . $apiLogin . "^" . $transId . "^" . $amount . "^";
        //  $this->generateSH512($textToHash,$signatureKey);

        //     debug(strtoupper($regs[68]));
        //     debug(strtoupper($this->generateSH512($textToHash,$signatureKey)));
        //     debug($transaction_response); exit;
        $isValidHash = true;
        if ($regs[0] == '1' || $regs[0] == '4') {
            if (strtoupper($regs[68]) != strtoupper($this->generateSH512($textToHash, $signatureKey))) {
                $error = 'general';
                $isValidHash = false;
            }
            if ($regs[0] == '4') {
        ...
        if (strtolower(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGGING) == 'true') {
            file_put_contents('_anet.log', var_export($regs, true) . PHP_EOL . var_export($_SESSION,
                    true) . PHP_EOL . date('c') . PHP_EOL . 'Hash validation:' . $isValidHash . PHP_EOL . '===========================================================' . PHP_EOL, FILE_APPEND);
        }
        if ($error != false) {
            tep_redirect('/' . tep_href_link(FILENAME_CHECKOUT, 'payment_error=' . $this->code . '&error=' . $regs[3], 'SSL'));
        }
    }
    ...
    function get_error()
    {
        ...
        $error = array(
            'title' => MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ERROR_TITLE,
            'error' => $error_message
        );
    ...
    static function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Authorize.net logs', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGGING', 'False', 'Do you want to log transactions Authorize.net in file? (log write in _anet.log)', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Signature Key', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_SIGNATURE_KEY', '', 'Signature key key used for encrypting data (https://support.authorize.net/s/article/What-is-a-Signature-Key)', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaction Server', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_SERVER', 'Test', 'Perform transactions on the live or test server. The test server should only be used by developers with Authorize.net test accounts.', '6', '0', 'tep_cfg_select_option(array(\'Live\', \'Test\'), ', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaction Mode', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_MODE', 'Test', 'Transaction mode used for processing orders', '6', '0', 'tep_cfg_select_option(array(\'Live\', \'Test\'), ', now())");
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Approved Transaction Order Status', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_ORDER_STATUS_ID', '0', 'Set the status of orders made with this payment module to this value', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Transaction on Review Order Status', 'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_REVIEW_ORDER_STATUS_ID', '0', 'Set the status of orders flagged as being under review to this value', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
    ...
    function remove()
    {
        tep_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", self::keys()) . "')");
    }

    static function keys()
    {
        return array(
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_STATUS',
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGGING',
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_LOGIN_ID',
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_TRANSACTION_KEY',
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_SIGNATURE_KEY',
            ...
            'MODULE_PAYMENT_AUTHORIZENET_CC_AIM_REVIEW_ORDER_STATUS_ID',
...
// Hacked by Lance Rushing
    ...
    function sendTransactionToGateway($url, $parameters)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            exec(escapeshellarg(MODULE_PAYMENT_AUTHORIZENET_CC_AIM_CURL) . ' -d ' . escapeshellarg($parameters) . ' "' . $server['scheme'] . '://' . $server['host'] . $server['path'] . (isset($server['query']) ? '?' . $server['query'] : '') . '" -P ' . $server['port'] . ' -k',
                $result);
            $result = implode("\n", $result);
        }
