...
\App\Logger\Log::channel('payment')->info('payment/callback/coinpayments', [
...
if (tep_not_null(MODULE_PAYMENT_COINPAYMENTS_MERCHANT) && tep_not_null(MODULE_PAYMENT_COINPAYMENTS_IPN_SECRET)) {
    ...
    if ($auth_ok) {
        $orders_id = (isset($HTTP_POST_VARS['invoice']) && is_numeric($HTTP_POST_VARS['invoice'])) ? (int)$HTTP_POST_VARS['invoice'] : 0;
        if ($orders_id > 0) {
            $order_query = tep_db_query("select orders_status, currency, currency_value from " . TABLE_ORDERS . " where orders_id = '" . $orders_id . "' and customers_id = '" . (int)$HTTP_POST_VARS['custom'] . "'");
            if (tep_db_num_rows($order_query) > 0) {
                ...
                $onePageCheckout->generateEmailByOrderId($orders_id);

                // send message only once. if we send email, change customer_notified to 1.
                $check_status_query = tep_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where customer_notified = 1 and orders_id = "' . $orders_id . '"');
                if (tep_db_num_rows($check_status_query) == 0) {
                    $onePageCheckout->createEmails($orders_id);
                    tep_db_perform('orders_status_history', array('orders_id' => $orders_id,
                ...
                if ($status < 100) {
                    ...
                    $comment_status .= '; (' . sprintf("%.08f", $HTTP_POST_VARS['amount1']) . ' ' .
                        $HTTP_POST_VARS['currency1'] . ' => ' . sprintf("%.08f", $HTTP_POST_VARS['amount2']) . ' ' . $HTTP_POST_VARS['currency2'] . ')';
                    ...
                    if ($HTTP_POST_VARS['amount1'] != number_format($total['value'] * $order['currency_value'], $currencies->get_decimal_places($order['currency']))) {
                        $comment_status .= '; transaction value (' . tep_output_string_protected($HTTP_POST_VARS['amount1']) . ') ' .
                            'does not match order value (' . number_format($total['value'] * $order['currency_value'], $currencies->get_decimal_places($order['currency'])) . ')';
