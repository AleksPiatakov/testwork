$(function () {
    ...
    $('#images > table.images').before($('#custom_form_wrapper').html());
    $('#images form').fadeIn();
...
//		url: 'html5uploader/post_file.php?act=update&pid='+pid3d+'&opid=',
        ...
        uploadFinished: function (i, file, response) {
            ...
            $.data(file).find('.delimg').click(function () {
                deleteCurrent(pid3d, $(this).attr('name'));
            });
            $.data(file).append('<span class="editimg" name="' + response['current'] + '"></span>');
            $.data(file).find('.editimg').click(function () {
                jsoncrop($(this).parent().find('img').attr('name'));
            });
    ...
    function showCurrent(pid, opid) {
        $.getJSON('html5uploader/post_file.php', {
            ...
            if (obj != null) {
                ...
                for (i = 0; i < objlen; i++) {
                    let fileTag = '';
                    let ext = obj[i].match('\.[a-zA-Z0-9]+$');
                    if (ext == ".mp4" || ext == ".avi" || ext == ".mov") {
                        $(curr_dropbox).append('<div class="preview done" id="' + obj[i] + '"><div class="imageHolder videoHolder ">' +
                            '<video name=' + obj[i] + ' src="../images/products/' + obj[i] + '" width="150" height="150" class="video-uploader" autoplay muted></video>' +
                            '<img src="../images/play.png" class="video-button" alt="">' +
                            '<span class="delimg" name="' + obj[i] + '"></span><span class="editimg" name="' + obj[i] + '"></span></div></div>');
                    } else {
                        $(curr_dropbox).append('<div class="preview done" id="' + obj[i] + '"><span class="imageHolder">' +
                            '<img name=' + obj[i] + ' src="../getimage/150x150/products/' + obj[i] + '">' +
                            '<span class="delimg" name="' + obj[i] + '"></span><span class="editimg" name="' + obj[i] + '"></span></div>');
                    }
                }
                $('.delimg').click(function () {
                    deleteCurrent(pid, $(this).attr('name'));
                });
    ...
    function deleteCurrent(pid, img) {
        ...
        var opid = $('.delimg[name="' + img + '"]').closest('.dropbox').attr('id').replace('dropbox_', '');
        ...
        $.getJSON('html5uploader/post_file.php', {
            ...
            $('.delimg[name="' + img + '"]').closest('.preview').animate({
                width: 0,
                marginLeft: 0,
                opacity: 0
            }, 500, function () {
...
//	$( "#crop_button" ).click(function() {
        ...
        $.getJSON('html5uploader/post_file.php', {
            ...
            $('img[name="' + img_name + '"]').attr('src', '../getimage/150x150/products/' + img_name + '?' + Math.random());
...
function jsoncrop(cur_image) {
    ...
    $('#crop_area').html('<img name="' + cur_image + '" src="../images/products/' + cur_image + '?' + Math.random() + '" />');
