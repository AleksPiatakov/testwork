              ...
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" valign="top" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cID=' . $_GET['cPath']) . '">' . tep_text_button(BUTTON_BACK_NEW) . '</a>&nbsp<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $_GET['cPath'] . '&action=store_file') . '">' . tep_text_button(BUTTON_ADMIN_FILES_NEW) . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table>
...
<?php 
  $installed_boxes_query = tep_db_query("select admin_files_name as admin_boxes_name, admin_files_id as admin_boxes_id from " . TABLE_ADMIN_FILES . " where admin_files_is_boxes = 1 order by admin_files_name");
  $installed_boxes = array();
  while($db_boxes = tep_db_fetch_array($installed_boxes_query)) {
    $boxes[] = array('admin_boxes_name' => $db_boxes['admin_boxes_name'],
                     'admin_boxes_id' => $db_boxes['admin_boxes_id']);
  }    
...
<?php
  ...
  switch ($_GET['action']) {  
    case 'store_file': 
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_save.gif', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $_GET['cPath']) . '">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>');
      break;
    case 'remove_file': 
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_confirm.gif', IMAGE_CONFIRM) . ' <a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $_GET['cPath'] . '&fID=' . $_GET['fID']) . '">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>');
      break;
    default:
      if (is_object($cInfo)) {
      ...
      if (is_object($fInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $_GET['cPath'] . '&action=store_file') . '">' . tep_text_button(BUTTON_ADMIN_FILES_NEW) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_FILES, 'cPath=' . $_GET['cPath'] . '&fID=' . $fInfo->admin_files_id . '&action=remove_file') . '">' . tep_text_button(BUTTON_ADMIN_REMOVE_NEW) . '</a>');
