...
<form class="form-horizontal" action="<?php echo ($_SERVER['SCRIPT_URL']?:$_SERVER['SCRIPT_NAME']) . '?action=' . $action_form;?>" method="post" enctype="multipart/form-data">
    ...
    <div class="col-md-3">
        ...
        <?php foreach ($data['allowed_fields'] as $field_name => $option): ?>
            ...
            <?php endif; ?>
        <?php endforeach; ?>
        <?php if(isset($data['option_names']) && key_exists(current($data['data'])['email_name'], $data['option_names'])): ?>
            <div ><?php echo addDoubleDot(TABLE_HEADING_OPTION_NAMES);?></div>
            <ul class="list-group">
            <?php foreach ($data['option_names'][current($data['data'])['email_name']] as $name => $const): ?>
                <li class="list-group-item"><?=$name.' : <span style="float: right;">{'.$const.'}</span>'?></li>
            <?php endforeach; ?>
            </ul>
        <?php endif; ?>
