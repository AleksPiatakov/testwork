 ...
 * Date: 01.12.2017
 * Time: 12:51
 * https://github.com/yiisoft/yii/blob/1.1.14/framework/utils/CFileHelper.php
 */

namespace admin\includes\solomono\app\models\languages_translater;
...
class yii {
    ...
    protected static function validatePath($base, $file, $isFile, $fileTypes, $exclude) {
        ...
        if (($type=pathinfo($file, PATHINFO_EXTENSION))!=='')
            return in_array($type, $fileTypes);else
            return false;
    }
    ...
    public static function getMimeTypeByExtension($file, $magicFile=null) {
        ...
        if ($magicFile===null && $extensions===null)
            $extensions=require(Yii::getPathOfAlias('system.utils.mimeTypes') . '.php');elseif ($magicFile!==null && !isset($customExtensions[$magicFile]))
            $customExtensions[$magicFile]=require($magicFile);
        if (($ext=pathinfo($file, PATHINFO_EXTENSION))!=='') {
            $ext=strtolower($ext);
            if ($magicFile===null && isset($extensions[$ext]))
                return $extensions[$ext];elseif ($magicFile!==null && isset($customExtensions[$magicFile][$ext]))
