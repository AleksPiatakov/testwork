...
function sbs_get_country_list($name, $selected = '', $parameters = '') { 
   ...
   return preg_replace('#class="[^"]*"#', '', tep_draw_pull_down_menu($name, $countries_array, $selected, $parameters));
}
    ...
    if ($is_read_only) {
      ...
      if ($entry_state_error) {
        if ($entry_state_has_zones) {
          ...
          echo preg_replace('#class="[^"]*"#', '', tep_draw_pull_down_menu('state', $zones_array)) . '&nbsp;' . ENTRY_STATE_ERROR;
        } else {
...
<?php
  if ($is_read_only) {
    ...
    echo preg_replace('#class="[^"]*"#', '', tep_draw_pull_down_menu('newsletter', $newsletter_array, $account['customers_newsletter'])) . '&nbsp;' . ENTRY_NEWSLETTER_TEXT;
  }
?></td>
          </tr>
        </table></td>
