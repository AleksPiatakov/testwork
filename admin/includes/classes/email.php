...
function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
  ...
  $to = (!empty($to_name) ? '"' . $to_name . '" <' . $to_addr . '>' : $to_addr);
  $from = (!empty($from_name) ? '"' . $from_name . '" <' . $from_addr . '>' : $from_addr);
...
// Make up the date header as according to RFC822
      ...
      $to = (!empty($to_name) ? 'To: "' . $to_name . '" <' . $to_addr . '>' : 'To: ' . $to_addr);
      $from = (!empty($from_name) ? 'From: "' . $from_name . '" <' . $from_addr . '>' : 'From: ' . $from_addr);
