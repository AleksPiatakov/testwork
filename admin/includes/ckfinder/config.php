...
$new_catalog = (!empty($env['CATALOG_FOLDER']) ? ('/' . $env['CATALOG_FOLDER']) : '');
/**
...
function CheckAuthentication()
{
    ...
    $adminCookie = array_filter($sessions,function($cookie){$cookie = explode('=',$cookie); return trim($cookie[0]) == 'osCAdminID';});
    if ($adminCookie){
        $rootPath = dirname(dirname(dirname(dirname(dirname(dirname(dirname($_SERVER['SCRIPT_FILENAME'])))))));
        require_once($rootPath.'/includes/bootstrap.php');
        $adminCookie = reset($adminCookie);
        $sId = explode('=',$adminCookie)[1];
        $link = mysqli_connect(getenv('DB_HOST'),getenv('DB_USERNAME'), getenv('DB_PASSWORD'),getenv('DB_DATABASE')) or die("Немає підключення до бази");
        $query_add = "select value from sessions where sesskey = '".$sId."'";
        $result_add = mysqli_query($link,$query_add,MYSQLI_USE_RESULT);
        $result = mysqli_fetch_assoc($result_add);
    ...
    //
    //        $link = mysql_connect($server, $db_user, $db_pass) or die("Немає підключення до бази");
    //        mysql_select_db($db_name, $link);
    //
    //        $query_add = "select value from sessions where sesskey = '".$osc[1]."'";
    //        $result_add = mysql_query($query_add, $link);
    //        $result = mysql_fetch_array($result_add);
    //
    //        $serach_ligin_array = explode(';',$result['value']);
    //        foreach($serach_ligin_array as $v) {
    //          print_r($v.'____');
    //          if (strpos($v, 'login_first_name') !== false) {
    //               return true;
    //          }
    //
    //        }
    ...
    // if ($_SESSION['login_first_name']!='') return true;
    // return isset($_SESSION['login_first_name']) && $_SESSION['login_first_name'];
    // return true;
}
...
$siteLink = '';
if (strpos($_SERVER["HTTP_REFERER"], 'type=Images&CKEditor=message') !== false) {
