...
$checkArttributes = isset($_REQUEST['checkArttributes']) && ($_REQUEST['checkArttributes'] == 'on');
...
//if(EXCEL_IMPORT_MODULE_ENABLED=='false') tep_redirect(tep_href_link(FILENAME_DEFAULT)); // если в данном магазине нет такого модуля то перекидываем на главную
...
define('EP_MODEL_NUMBER_SIZE', 255); // default is 12
...
//  //  $ep_separator = ';';  // semi-colon
//  } else {
//    $ep_separator = ';';  // semi-colon // default for all others.
//  }
//}
...
if($_GET['separators'])
{
  if($_GET['separators'] == '2') $ep_separator = ',';
  else $ep_separator = ';';
  unset($_GET['separators']);
}

if($_POST['separators'])
{
  if($_POST['separators'] == '2') $ep_separator = ',';
  else $ep_separator = ';';
  unset($_POST['separators']);
}
...
define('EP_FROOGLE_PRODUCT_INFO_PATH', HTTP_SERVER . DIR_WS_CATALOG . "product_info.php");

// **** Froogle product image path ****
// Set this to the path to your images directory
define('EP_FROOGLE_IMAGE_PATH', HTTP_SERVER . DIR_WS_CATALOG_IMAGES);
...
if ($current_page == FILENAME_IMPORT_EXPORT) {
  ...
  include(DIR_FS_CATALOG.'ext/import/import.php');
...
elseif ($current_page == FILENAME_NEW_IMPORT_EXPORT) {
  ...
  include(DIR_FS_CATALOG.'ext/import/new_import.php');
}
?>
