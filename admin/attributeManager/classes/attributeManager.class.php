...
// raid 
      ...
      if ($id == 0){
        $id = 1;
			...
			$arrData = array (
				...
				'products_options_name' => amDB::input($this->utf8_JSON_russian_cyr($name))
			);
			if (AM_USE_SORT_ORDER) {
	...
	function addOptionValue($get) {
		...
		foreach($arrOptionValues as $optionValue) {
			...
			$ovData = array (
				...
				'language_id' => amDB::input($langId), 
				'products_options_values_name' => amDB::input($this->utf8_JSON_russian_cyr($name))
			);
	...
	function getAllTemplatesAndAttributes() {
		if(0 === count($this->arrAllTemplatesAndAttributes)) {
          ...
          $this->arrAllTemplatesAndAttributes[$templateID]['pa_qty'][$res['option_values_id']] = $res['pa_qty'];
          $this->arrAllTemplatesAndAttributes[$templateID]['pa_article'][$res['option_values_id']] = $res['pa_article'];
	...
	function saveTemplate($get) {
		...
		if(0 !== count($allProdOptionsAndValues)) {
			if(0 != $newTemplateId) {
				...
				$data = array(
					'template_name' => $this->utf8_JSON_russian_cyr($templateName)
				);
					...
					foreach ($values['values'] as $optionValuesId => $allOptionValues){
						if (!AM_USE_SORT_ORDER) {
  							$data = array(
                                ...
                                'pa_qty' => $values['values'][$optionValuesId]['pa_qty'],
			...
			foreach ($allTemplatesAttributes[$templateId] as $optionsId => $values) {
				...
				if(in_array($optionsId,$allOptions)) {
					if(is_array($values)) {
                      ...
                      'pa_qty' => $allTemplatesAttributes[$templateId]['pa_qty'][$optionValuesId],
                      'pa_article' => $allTemplatesAttributes[$templateId]['pa_article'][$optionValuesId],
...
//								  echo 'TempID '.$templateId . ' OPTvalID ' . $optionValuesId . ' ';
	...
	function buildOptionValueDropDown($optionId = null, $subtract = true) {
		...
		if(true === $subtract) {
			...
			if(is_array($allProductsOptionsAndValues) &&  0 !== count($allProductsOptionsAndValues)){
