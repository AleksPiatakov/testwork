...
function amUpdate(optionId, optionValueId, optionSender) {
  ...
  pa_qty=getDropDownValue('pa_qty_'+optionValueId);
  pa_article=getDropDownValue('pa_article_'+optionValueId);
	...
	amSendRequest('amAction=update&option_id='+optionId+'&option_value_id='+optionValueId+'&price='+getDropDownValue('price_'+optionValueId)+'&prefix='+getDropDownValue('prefix_'+optionValueId)+'&pa_qty='+getDropDownValue('pa_qty_'+optionValueId)+'&pa_article='+getDropDownValue('pa_article_'+optionValueId)+'&sortOrder='+getDropDownValue('sortOrder_'+optionValueId)+'&weight='+getDropDownValue('weight_'+optionValueId)+'&weight_prefix='+getDropDownValue('weight_prefix_'+optionValueId),'',false);
...
function amAddAttributeToProduct() {
  ...
  var pa_qty = getDropDownValue('newpa_qty');
  var pa_article = getDropDownValue('newpa_article');
...
//  alert(pa_qty);
	...
	if(0 == option || 0 == optionValue)
		return false;
	amSendRequest('amAction=addAttributeToProduct&option_id='+option+'&option_value_id='+optionValue+'&prefix='+pricePrefix+'&pa_qty='+pa_qty+'&pa_article='+pa_article+'&price='+price+'&sortOrder='+sortOrder+'&weight_prefix='+weightPrefix+'&weight='+weight);
	return false;
}
...
function amShowOptionValueList(optionId) {
	getElement('add_selected_'+optionId).style.display = "none";
	getElement('new_option_value_'+optionId).style.display = "";
	getElement('new_option_value_button_'+optionId).style.display = "";
}

function amShowNewAttributePanel(optionId) {
	if(getElement('new_attribute_panel').style.display){
		getElement('new_attribute_panel').style.display = "";
	} else {
		getElement('new_attribute_panel').style.display = "none";
	}
}
...
function getAllPromptTextValues() {
	...
	for (var i = 0; i < allValues.length; i++) 
		if('text' == allValues[i].type) 
			returnArray.push(allValues[i].id+':'+encodeURI(getElement(allValues[i].id).value));
