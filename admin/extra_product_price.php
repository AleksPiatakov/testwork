      ...
      <tr><form name="extra_product_price_form" <?php echo 'action="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo tep_draw_hidden_field('sID', $_GET['sID']); ?>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main" align="right" valign="top"><br><?php echo (($form_action == 'insert') ? tep_image_submit('button_insert.gif', IMAGE_INSERT) : tep_image_submit('button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $_GET['sID']) . '">' . tep_text_button(BUTTON_CANCEL_NEW); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<?php
    ...
    $extra_product_price_split = new splitPageResults($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS, $extra_product_price_query_raw, $extra_product_price_query_numrows);
...
?>
              <tr>
                <td colspan="7"><table border="0" width="100%" cellpadding="0"cellspacing="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $extra_product_price_split->display_count($extra_product_price_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_EXTRA_PRODUCT_PRICE); ?></td>
                    <td class="smallText" align="right"><?php echo $extra_product_price_split->display_links($extra_product_price_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
<?php
  if (empty($action)) {
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_text_button(BUTTON_NEW_SALE_NEW) . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'copy':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_copy.gif', IMAGE_COPY) . '&nbsp;<a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $sInfo->extra_product_price_id) . '">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>');
      break;
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $sInfo->extra_product_price_id) . '">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>');
      break;
    default:
      if (is_object($sInfo)) {
        $heading[] = array('text' => '<b>' . $sInfo->extra_product_price_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $sInfo->extra_product_price_id . '&action=edit') . '">' . tep_text_button(BUTTON_EDIT_NEW) . '</a> <a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $sInfo->extra_product_price_id . '&action=copy') . '">' . tep_text_button(BUTTON_COPY_TO_NEW) . '</a> <a href="' . tep_href_link(FILENAME_EXTRA_PRODUCT_PRICE, 'page=' . $_GET['page'] . '&sID=' . $sInfo->extra_product_price_id . '&action=delete') . '">' . tep_text_button(BUTTON_DELETE_NEW) . '</a>');
