...
//--></script>
      ...
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
			...
			<td class="main"><?php
                if(file_exists(DIR_FS_EXT . 'customers_groups/customers_groups.php')) {
                    ...
                    if ($manudiscount['manudiscount_groups_id'] != 0) {
                        ...
                        ?><input type="checkbox" name="checkbox_customers_groups" checked value="false"
                                 onClick="if (manudiscount_groups_id.disabled && checkbox_customers_groups.checked) {checkbox_customers_groups.value = true; manudiscount_groups_id.disabled = false; manudiscount_customers_id.disabled = true; checkbox_customers.checked = false} else {checkbox_customers_groups.value = false; manudiscount_groups_id.disabled = true; manudiscount_customers_id.disabled = true; }"><?php
                    } else {
		  ...
		  <tr>
			...
			<td class="main"><?php
		          $customers_groups_query = tep_db_query("select distinct manufacturers_name, manufacturers_id from " . TABLE_MANUFACTURERS_INFO . " where languages_id = $languages_id  order by manufacturers_name");
      ...
      <tr>
        <td align="right" class="main"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('action','cID'))) .'">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>'; ?></td>
      </tr>
      </form>
...
?>
                  <tr>
                    <td align="right" colspan="2"><?php echo '<a href="' . tep_href_link('manudiscount.php') . '">' . tep_text_button(BUTTON_RESET_NEW) . '</a>'; ?></td>
                  </tr>
<?php
    } else {
?>
			      <tr>
                    <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . tep_href_link('manudiscount.php', 'page=' . $_GET['page'] . '&action=new') . '">' . tep_text_button(BUTTON_INSERT_NEW) . '</a>'; ?></td>
                  </tr>
<?php
	}
?>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'confirm':
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id) . '">' . tep_text_button(BUTTON_CANCEL_NEW) . '</a>');
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<b>' . $cInfo->manudiscount_name . ' </b>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id . '&action=edit') . '">' . tep_text_button(BUTTON_CANCEL_EDIT) . '</a> <a href="' . tep_href_link('manudiscount.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->manudiscount_id . '&action=confirm') . '">' . tep_text_button(BUTTON_DELETE_NEW) . '</a>');
