...
includeLanguages(DIR_WS_LANGUAGES . $language . '/' . FILENAME_WISHLIST);
...
if (!empty($_GET['delete_prod'])) {
    $wishList->remove($_GET['delete_prod']);
}
//delete all
if ($_GET['delete'] == 'all') {
    $wishList->reset(true);
...
if (isset($_POST['email_prod_x'])) {
    ...
    if (tep_session_is_registered('customer_id')) {
        ...
        $subject = $customer['customers_firstname'] . ' ' . sprintf(WISHLIST_EMAIL_SUBJECT, WISHLIST_EMAIL_SUBJECT);
        $link = HTTP_SERVER . DIR_WS_CATALOG . FILENAME_WISHLIST . "?public_id=" . $customer_id;

        //REPLACE VARIABLES FROM DEFINE
        $arr1 = array('$from_name', '$link');
        $arr2 = array($from_name, $link);
        $replace = str_replace($arr1, $arr2, sprintf(WISHLIST_EMAIL_LINK, STORE_NAME));
        ...
        $subject = $from_name . ' ' . sprintf(WISHLIST_EMAIL_SUBJECT, WISHLIST_EMAIL_SUBJECT);
        ...
        $body = $message . "\n\n" . $prods . "\n\n" . sprintf(WISHLIST_EMAIL_GUEST, STORE_NAME);
    }
