...
class ot_gv
{
    ...
    function __construct()
    {
        ...
        $this->sort_order = MODULE_ORDER_TOTAL_OT_GV_SORT_ORDER;
        ...
        if ($_POST['gv_redeem_code']) {
            ...
            if (tep_db_num_rows($gv_query) != 0) {
                ...
                if ((tep_db_num_rows($redeem_query) != 0) && ($gv_result['coupon_type'] == 'G')) {
                    tep_redirect(tep_href_link(FILENAME_CHECKOUT, 'error_message=' . urlencode(ERROR_NO_INVALID_REDEEM_GV), 'SSL'));
                }
            }
            if ($gv_result['coupon_type'] == 'G') {
                ...
                tep_redirect(tep_href_link(FILENAME_CHECKOUT, 'error_message=' . urlencode(ERROR_REDEEMED_AMOUNT . $currencies->format($gv_amount)), 'SSL'));
            }
        }
        if ($_POST['submit_redeem_x'] && $gv_result['coupon_type'] == 'G') {
            tep_redirect(tep_href_link(FILENAME_CHECKOUT, 'error_message=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
        }
    }
...
//          $order->info['total'] -= $tod_amount;   //// ????? Strider
    ...
    static function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Порядок сортировки', 'MODULE_ORDER_TOTAL_OT_GV_SORT_ORDER', '740', 'Порядок сортировки модуля.', '6', '2', now())");
