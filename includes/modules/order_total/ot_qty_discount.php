...
class ot_qty_discount
{
    ...
    function __construct()
    {
        ...
        $this->enabled = ((MODULE_QTY_DISCOUNT_STATUS == 'true') ? true : false);
        $this->sort_order = MODULE_ORDER_TOTAL_OT_QTY_DISCOUNT_SORT_ORDER;
    ...
    function process()
    {
        ...
        if ($od_amount > 0) {
            if (MODULE_QTY_DISCOUNT_RATE_TYPE == 'percentage') {
                $title_ext = ' (' . $this->calculate_rate($cart->count_contents()) . '%)';
            }
    ...
    {
        return array('MODULE_QTY_DISCOUNT_STATUS', 'MODULE_ORDER_TOTAL_OT_QTY_DISCOUNT_SORT_ORDER', 'MODULE_QTY_DISCOUNT_RATE_TYPE', 'MODULE_QTY_DISCOUNT_RATES', 'MODULE_QTY_DISCOUNT_INC_SHIPPING', 'MODULE_QTY_DISCOUNT_INC_TAX', 'MODULE_QTY_DISCOUNT_CALC_TAX');
    }
    ...
    static function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Порядок сортировки', 'MODULE_ORDER_TOTAL_OT_QTY_DISCOUNT_SORT_ORDER', '999', 'Порядок сортировки модуля.', '6', '2', now())");
