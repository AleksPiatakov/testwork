...
class ot_shipping
{
    ...
    function __construct()
    {
        ...
        $this->sort_order = MODULE_ORDER_TOTAL_OT_SHIPPING_SORT_ORDER;
    ...
    function process()
    {
        global $order, $currencies,$currency, $free_shipping, $shipping_modules, $shipping;
  ...
  /* One Page Checkout - BEGIN */
     ...
     //        print_r($pass);
            if (($pass == true) && ( ($order->info['total'] - ($order->info['shipping_cost'] !== null ?: 0)) >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER)) {
                $r_pass = true;
                $free_shipping = true;
                $order->info['shipping_method'] = $order->info['shipping_method'] . '.<br /> ' . FREE_SHIPPING_TITLE . '';
  ...
  /* One Page Checkout - END */
        ...
        if (tep_not_null($order->info['shipping_method'])) {
            // Check if haven't enable module recalculate order total and not show ot_shipping
            $enabledShippingModules = $shipping_modules->quote();
            if (empty($enabledShippingModules)) {
                $shipping = null;
                $order->cart();
                return;
            }
            ...
            if ($r_pass == true) {
                $r_text = '&nbsp;';
                $r_title = $order->info['shipping_method'] . ' ' . $currencies->format(MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER, true, $currency)/*.' '.($currencies->currencies[$currency]['symbol_left']?$currencies->currencies[$currency]['symbol_left']:$currencies->currencies[$currency]['symbol_right'])*/;
            } else {
                $r_text = $currencies->format($order->info['shipping_cost'], true, $order->info['currency'], $order->info['currency_value']);
                $r_title = $order->info['shipping_method'] . ': ';
            }

            if (getConstantValue('MODULE_SHIPPING_NWPOSHTANEW_SHOW_SHIPPING_COST_STATUS', 'false') === 'false' && $order->info['shipping_method_code'] === 'nwposhtanew') {
                $this->output[] = array(
                    'title' => $r_title,
                    'text' => '',
                    'value' => $order->info['shipping_cost']
                );
            } elseif (!empty($GLOBALS[$module]->costText)) {
                $this->output[] = array(
                    'title' => $r_title,
                    'text' => $GLOBALS[$module]->costText,
                    'value' => $order->info['shipping_cost']
                );
            } else {
                $this->output[] = array(
    ...
    {
        return array('MODULE_ORDER_TOTAL_SHIPPING_STATUS', 'MODULE_ORDER_TOTAL_OT_SHIPPING_SORT_ORDER', 'MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING', 'MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER', 'MODULE_ORDER_TOTAL_SHIPPING_DESTINATION');
    }
    ...
    static function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Порядок сортировки', 'MODULE_ORDER_TOTAL_OT_SHIPPING_SORT_ORDER', '2', 'Порядок сортировки модуля.', '6', '2', now())");
