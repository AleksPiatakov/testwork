...
*/
    function _match($token)
    {
        switch ($token) {
            ...
            // if it's a reference
                if (
                    preg_match('/^[A-I]?[A-Z][0-9]+$/i', $token) and
                ...
                elseif (
                    preg_match('/^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$/i', $token) and
                ...
                elseif (preg_match('/^[A-Z0-9А-Ь\.]+$/i', $token) and ($this->_lookahead == "(")) {
                    return($token);
                }
                return '';
        }
...
*/
    function _fact()
    {
      ...
      // if it's a range
        elseif (
            preg_match('/^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$/i', $this->_current_token) or
      ...
      // if it's a function call
        elseif (preg_match('/^[A-Z0-9А-Ь\.]+$/i', $this->_current_token)) {
            $result = $this->_func();
            return($result);
        }
