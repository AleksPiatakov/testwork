...
// class constructor
    function __construct()
    {
        ...
        $this->sort_order = getConstantValue('MODULE_SHIPPING_ZONES_SORT_ORDER', '0');
        $this->icon = '';
...
// class methods
    function quote($method = '')
    {
        ...
        if ($error == true) {
            ...
            $this->quotes = false; // MODULE_SHIPPING_ZONES_INVALID_ZONE;
        }
    ...
    {
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones method', 'MODULE_SHIPPING_ZONES_STATUS', 'true', 'Do you want to enable Zones method?', '6', '0', 'tep_cfg_select_option_checkbox(array(\'true\', \'false\'), ', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax', 'MODULE_SHIPPING_ZONES_TAX_CLASS', '0', 'Use tax.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort order', 'MODULE_SHIPPING_ZONES_SORT_ORDER', '0', 'Enter sort order for this module.', '6', '0', now())");
        for ($i = 1; $i <= self::$num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'US,CA';
            }
            tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Countries for zone " . $i . "', 'MODULE_SHIPPING_ZONES_COUNTRIES_" . $i . "', '" . $default_countries . "', 'Comma separated countries list for zone " . $i . ".', '6', '0', now())");
            tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Shipping cost for zone " . $i . "', 'MODULE_SHIPPING_ZONES_COST_" . $i . "', '3:8.50,7:10.50,99:20.00', 'Delivery cost is comma separated for zone " . $i . " based on the maximum cost of the order. For example: 3: 8.50,7: 10.50, ... This means that the shipping cost for orders, weighing up to 3 kg will cost 8.50 for buyers from the countries of the " . $i . " zone.', '6', '0', now())");
            tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Cost for zone " . $i . "', 'MODULE_SHIPPING_ZONES_HANDLING_" . $i . "', '0', 'The cost of using this method of delivery.', '6', '0', now())");
        }
    }
