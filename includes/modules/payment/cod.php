...
// class constructor
    function __construct()
    {
        ...
        $this->code = 'cod';
        $this->title = MODULE_PAYMENT_COD_TEXT_TITLE . (!isMobile() ? MODULE_PAYMENT_COD_IMAGES : '');
...
// class methods
    function update_status()
    {
        ...
        if (
            $this->enabled == true &&
            (int)getConstantValue('MODULE_PAYMENT_COD_ZONE', 0) > 0 &&
            getConstantValue('ACCOUNT_COUNTRY') == 'true' && getConstantValue('ACCOUNT_STATE') == 'true'
        ) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)MODULE_PAYMENT_COD_ZONE . "' and (zone_country_id = '" . (int)$order->billing['country']['id'] . "' or zone_country_id=0) order by zone_id");
