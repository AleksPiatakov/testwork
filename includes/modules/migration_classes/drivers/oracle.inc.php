...
ORDER BY 1"); //! views don't have schema
    }
    ...
    function table_status($name = "")
    {
        ...
        $search = q($name);
        foreach (
            get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = ' . q(DB) . ($name != "" ? " AND table_name = $search" : "") . "
...
ORDER BY 1") as $row
    ...
    function indexes($table, $connection2 = null)
    {
        $return = array();
        foreach (
            get_rows("SELECT uic.*, uc.constraint_type
