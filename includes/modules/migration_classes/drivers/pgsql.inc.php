...
" . ($name != "" ? "AND relname = " . q($name) : "ORDER BY relname")) as $row
    ...
    function fields($table)
    {
        ...
        foreach (
            get_rows("SELECT a.attname AS field, format_type(a.atttypid, a.atttypmod) AS full_type, pg_get_expr(d.adbin, d.adrelid) AS default, a.attnotnull::int, col_description(c.oid, a.attnum) AS comment, $identity_column AS identity
FROM pg_class c
...
AND typelem = 0");
    ...
    function create_sql($table, $auto_increment, $style)
    {
        ...
        foreach ($indexes as $index_name => $index) {
            switch ($index['type']) {
                case 'UNIQUE':
                    $return_parts[] = "CONSTRAINT " . idf_escape($index_name) . " UNIQUE (" . implode(', ', array_map('idf_escape', $index['columns'])) . ")";
                    break;
                case 'PRIMARY':
