...
###################################################################################*/
    function __construct($cart_info, $cache_time = PAGE_CACHE_LIFETIME, $debug_switch = PAGE_CACHE_DEBUG_MODE)
    {
        global $SID;
        ...
        $this->cache_language = '_' . tep_db_prepare_input($_SESSION['language']);
        $this->debug_output['cache_language'] = $this->cache_language;
        # Set the cache file currency and store it
        $this->cache_currency = '_' . tep_db_prepare_input($_SESSION['currency']);
        ...
        # collect the garbage
        if (rand(0, 100) <= 5) {
            $this->collect_garbage(); // 5% chance of triggering GC
        }
