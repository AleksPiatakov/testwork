...
// class constructor
    function __construct()
    {
        ...
        if (defined('MODULE_ORDER_TOTAL_INSTALLED') && tep_not_null(MODULE_ORDER_TOTAL_INSTALLED)) {
            $this->modules = array_diff(explode(';', MODULE_ORDER_TOTAL_INSTALLED), ['']);
        ...
        if (is_array($this->modules)) {
            ...
            foreach ($this->modules as $key => $value) {
                ...
                if ($GLOBALS[$class]->enabled) {
                    ...
                    for ($i = 0, $n = sizeof((array)$GLOBALS[$class]->output); $i < $n; $i++) {
                ...
                if ($GLOBALS[$class]->enabled) {
                    ...
                    for ($i = 0; $i < $size; $i++) {
                        $output_string .= '              <tr class="' . $class . '">' . "\n" .
                                '                <td align="left" class="main" style="padding: 3px;">' . $GLOBALS[$class]->output[$i]['title'] . '</td>' . "\n" .
                                '                <td align="left" class="main" style="padding: 3px;">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                                '              </tr>';
                    }
                }
            }
...
//
    function credit_selection()
    {
        ...
        if (MODULE_ORDER_TOTAL_INSTALLED) {
            ...
            $header_string .= '     <tr><td><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n";
            ...
            foreach ($this->modules as $key => $value) {
                ...
                if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class) {
                    ...
                    if (!empty($use_credit_string) || !empty($selection_string)) {
                ...
                if (($GLOBALS[$class]->enabled && $GLOBALS[$class]->credit_class)) {
                    ...
                    if ($_POST[$post_var]) {
                        if (!tep_session_is_registered($post_var)) {
                            ...
                            $post_var = tep_db_prepare_input($_POST[$post_var]);
                    }
