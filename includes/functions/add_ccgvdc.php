...
function create_coupon_code($salt = "secret", $length = 10)
{
 // Set the length of the redeem code, the longer the more secure
    ...
    while ($good_result == 0) {
        ...
        $query = tep_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_code = '" . tep_db_prepare_input($id1) . "'");
...
function tep_gv_account_update($customer_id, $gv_id)
{
   ...
   // new code bugfix
        $gv_query = tep_db_query("update " . TABLE_COUPON_GV_CUSTOMER . " set amount = '" . tep_db_prepare_input($new_gv_amount) . "' where customer_id = " . (int)$customer_id);
...
{
    $tax_query = tep_db_query("select tax_rate from " . TABLE_TAX_RATES . " where tax_description = '" . tep_db_prepare_input($tax_desc) . "'");
